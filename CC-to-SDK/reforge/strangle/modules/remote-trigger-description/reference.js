...
//
// The prompt half of a pair: the tool object's `description()` returns a
// different, shorter constant declared two slots earlier in the same `var`
// statement, and it is `prompt()` — this one — that reaches a request body. The
// row's `declarator` index is part of its signature for exactly that reason.
//
// RemoteTrigger's `call` is an excluded server boundary (§1.2), so this text is
// the entire observable surface of the tool and owning it is the whole of what
// the campaign can own here.
export function remoteTriggerDescription() {
