...
//
// ZERO free variables and one template literal, the same shape as W7.5's prompt
// sections: `captures: []` on its row is a positive claim the build re-derives
// every time and `strangle/perturb.ts` requires to be the complete inventory.
export function enterWorktreeDescription() {return`Use this tool ONLY when explicitly instructed to work in a worktree \u2014 either by the user directly, or by project instructions (CLAUDE.md / memory). This tool creates an isolated git worktree and switches the current session into it.
