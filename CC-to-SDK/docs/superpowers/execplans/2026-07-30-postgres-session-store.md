...
- Decision: factory function `createPostgresSessionStore(client, opts)` (not a class), exported from the `src/index.ts` barrel alongside `PgLike`, options type, DDL, and ensure-helper; the Redis adapter stays untouched.
  ...
  Date/Author: 2026-07-30, grill.
- Decision (M4, review adjudication): the sidecar folds FROM THE TABLE — a `folded_id BIGINT` watermark column; each fold consumes entry rows with `id > folded_id` in id order, not the in-memory batch — and `delete()` removes the sidecar row before the entry rows.
...
Schema (two tables per prefix `p`, default prefix `ccs`, validated against `/^[A-Za-z_][A-Za-z0-9_]*$/` since identifiers cannot be parameterized):
    ...
    CREATE TABLE IF NOT EXISTS <p>_sessions (
      ...
      folded_id   BIGINT NOT NULL,   -- fold watermark: entry rows with id > folded_id are not yet in summary
...
- 2026-07-30 (M4): codex review returned 2×P1 + 2×P2, all adjudicated real; the sidecar was redesigned to fold-from-the-table over a `folded_id` watermark (fixes both P1s at the root — see Decision Log), delete order flipped, prefix length capped at 40. Sessions schema gained `folded_id`; the batch-RETURNING/`fresh` machinery was deleted (fold no longer consumes the batch). New watermark-recovery regression test; 18/18 adapter tests, unit suite 1191/1191, typecheck clean.
