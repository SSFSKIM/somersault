...
| **P83 ✅** | Are nested assistant messages' `usage` blocks summable into `Done (N tool uses · Xk tokens · Ys)`? Is there a name or type beyond `parent_tool_use_id`? → **not summable** (summing overshoots 265–342%); tool count and duration are exact from child frames, tokens come from `system/task_notification.usage`; identity is `subagent_type` + `task_description` on every child frame plus the `task_started` `tool_use_id ↔ task_id` join. See `11-p83-agent-usage-identity.md`. | 01#16, 02#44 | LT17, TR39, DG21 | done |
