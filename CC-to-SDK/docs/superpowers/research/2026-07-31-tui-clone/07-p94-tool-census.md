...
**Final gate probe-source SHA-256:** `4e747ef99dfb17b0192c9792cb9ba6c84838878801bca64ba80e482bfb1a18d1`
**Probe 94b final-source SHA-256:** `04c6d1a62c1ad665596aae385aa9eccdfaa3d498d2fff5e4c518f457669b5b79`
...
Second, terminal result frames are shape-checked before ownership or health aggregation. Required `subtype`, boolean `is_error`, string `result`, optional nonempty UUID, declared origin object, string-array `errors`, and `number | null` `api_error_status` are validated; malformed frames remain counted but can never become originating evidence. The exact final sources passed both self-tests, TypeScript compilation, forced safe-lifecycle and empty-case regressions, the secure directed Write case, all three no-tools correlation cases, and a sandboxed natural case that completed four Bash calls plus one Read. The final Write and natural reports each recorded `resolvedApiProviders:["firstParty"]`, zero malformed results, and healthy UUID-owned completion. Every P94b case contained `apiProvider:"firstParty"`; all six result frames were valid and healthy. All final stderr files were empty.
