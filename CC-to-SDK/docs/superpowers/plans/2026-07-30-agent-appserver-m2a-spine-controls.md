...
- [ ] **Step 2 (controller, keyed):** `cd CC-to-SDK/harness && set -a && source ../.env && set +a && npx vitest run test/live/appserver-m2-router-smoke.test.ts` — PASS. **Use `npx vitest run <file>`, NOT `npm run test:live -- <file>`:** the script is `vitest run test/live`, so an appended path is a second filter pattern and the whole keyed live tree (~34 files, all billing real API calls) runs. Same hazard applies to `npm run test:unit -- <file>`.

**Ordering note (controller):** as written this task's smoke used `thread/*/set`, which lands in Task 9 — so it runs in two halves. The engine-sourced half (init latch, usage route, limits route, live userMessage stitch, and the open question of whether the engine carries settings on a frame at all) runs HERE, because the risk it targets — a route condition that is wrong against the real engine — is the risk that has already materialized three times, and it needs no setter. The client-set half (a `thread/model/set` observed as `thread/settings/changed` with `source: "client"`) runs as part of Task 9's verification.
