...
- **Names:** `toolName` regex `/^[A-Za-z][A-Za-z0-9_-]{0,63}$/`; namespace `dyn` RESERVED; duplicate namespace names across the declaration set refused. Collisions are checked for **every name `overlayServerNames` would produce — `dyn` included when bare functions exist** — against the EFFECTIVE occupied set: `Object.keys(config.mcpServers ?? {})` ∪ `Object.keys((config.extraOptions?.mcpServers as object) ?? {})` ∪ `INJECTED_SERVER_NAMES` (the in-process servers `Session` adds AFTER `resolveOptions` — enumerate them at the Session build site, export the const beside the injection code, and reserve them UNCONDITIONALLY even when their feature flags are off: a name that might be injected on the next engine swap is not a name a declaration may hold; state that reasoning in the const's comment). **Comparison is on CANONICAL model-visible names** (r4 finding 3): the SDK normalizes every character outside `[A-Za-z0-9_-]` to `_` in `mcp__<server>__` names, so normalize each occupied name with that rule before comparing — a configured server `ops.prod` occupies `ops_prod`, and a declaration of `ops_prod` collides with it. Servers the SDK discovers from settings/plugins under non-strict MCP config CANNOT be enumerated at thread/start — that residual is a PUBLISHED bound (spec rev 3), not a refusal. Function names are checked against `NATIVE_TOOL_NAMES` — derived from a pinned `NATIVE_TOOL_MAP: Record<string, string>` (interface name → runtime tool name, e.g. `FileEditInput → Edit`, `FileReadInput → Read`, `FileWriteInput → Write`), whose KEY SET a drift test asserts equal to the interfaces parsed from the vendored **`sdk-tools.d.ts` `ToolInputSchemas` union** (45 members at 0.3.237 — NOT `sdk.d.ts`, which carries none; r4 finding 4) — an SDK bump adding a tool forces a mapping decision instead of silently opening a shadowing hole. Every declaration refusal is `-32602` naming the offender.
...
- **`record.config` NEVER holds runtime handles.** Live low-level `Server` instances ride only a TRANSIENT engine config (`{...cfg, dynamicToolServers}`) handed to the factory; `record.config` stays the serializable base. Serializable truth: `ThreadRecord.dynamicTools`. **`review/start` deliberately does NOT inherit declarations** (spec rev 3 — the review thread is a derived analysis engine, not the client's tool runtime; declarations live outside config precisely so review inheritance is structurally impossible): a row drives real `review/start` on a declaring target and asserts the review engine's config has NO `dynamicToolServers` and the review record no `dynamicTools`.
...
`ConvertOk.schema` is a built `z.object(...)` — `.strict()` when `additionalProperties === false`, `.passthrough()` when absent or `true`. **Its ONLY consumer is Task 5's `CallToolRequestSchema` handler** (argument validation); it never touches the advertisement — the low-level Server returns the declared JSON Schema verbatim (plan header).
...
**Value-domain totality (r3 finding 7): conversion NEVER throws.** Every keyword's VALUE is validated before use — `properties` must be a plain object of plain-object fields (`properties: null` refuses `properties:<non-object>`, not a walker crash); `minimum`/`maximum` must be finite numbers; **`minLength`/`maxLength` must be NON-NEGATIVE INTEGERS** (JSON Schema's own requirement — a `-1` or `1.5` would be advertised verbatim as an invalid schema while zod interprets it differently, splitting the advertised and enforced contracts; r4 finding 5); a `min > max` pair (either kind) refuses as inconsistent; `items` must be a plain object; `enum` must be a non-empty array whose members match the field `type`; `const` a string/number/boolean. Wrap the walk defensively so an unforeseen shape still returns `{ok:false, keyword:"<path>: unsupported shape"}` rather than throwing into a `-32603`.
...
export const NATIVE_TOOL_MAP: Readonly<Record<string, string>>;  // sdk-tools.d.ts interface name → runtime tool name (FileEditInput→Edit, …); the drift test asserts its KEYS equal the ToolInputSchemas union members, so an SDK bump forces a mapping decision
...
- rev 5 (2026-08-24): plan re-review round 4 — 5 findings; 4 accepted, 1 inverted. Stale `tool()`/`createSdkMcpServer` instructions swept from plan AND spec (one consistent low-level path); the review-inheritance demand INVERTED after re-reading the spec's intent — `review/start` deliberately does NOT inherit declarations (the structural exclusion is the design; now stated in the spec and pinned by a real review/start row) — the reviewer had read the protective clause as an inheritance promise; occupied-name collision comparison moved to CANONICAL model-visible names (SDK normalizes non-`[A-Za-z0-9_-]` to `_`; `ops.prod` occupies `ops_prod`); the catalog drift test re-targeted to `sdk-tools.d.ts`'s `ToolInputSchemas` union via a pinned `NATIVE_TOOL_MAP` (interface→runtime name; `sdk.d.ts` carries zero tool-input interfaces — verified) whose key set the test holds equal; `minLength`/`maxLength` restricted to non-negative integers with range-consistency refusal.
