...
export const RUNTIME_ONLY_NATIVE: readonly string[];             // native tools with NO interface in the union — "Skill" and "ToolSearch" at minimum (r6 finding 6: both are live native tools in this SDK; the union misses runtime-only ones); comment the provenance
...
Shapes per the spec (toolName regex + the no-`__` refine; description `.max(MAX_TOOL_DESCRIPTION_CHARS)`; tagged namespace children). Handler helper: refuse client `config.dynamicToolServers`/`extraOptions.dynamicToolServers` (server-owned); `validateDeclarations(specs, occupiedNames)` with `occupiedNames = [...mcpServers keys, ...extraOptions.mcpServers keys, ...INJECTED_SERVER_NAMES]`; stamp `record.dynamicTools` (the field exists since Task 7); regenerate the artifact — **assert `stable.results["tool/callResult"]` is present in the schemaGen expectations** (r6 finding 7).
