...
Shapes per the spec (toolName regex + the no-`__` refine; description `.max(MAX_TOOL_DESCRIPTION_CHARS)`; tagged namespace children). Handler helper: refuse client `config.dynamicToolServers`/`extraOptions.dynamicToolServers` (server-owned); `validateDeclarations(specs, occupiedNames)` with **`occupiedNames` derived by `resolveOptions`' REPLACEMENT semantics, not a union** (r7 finding 6: an own-property `extraOptions.mcpServers` REPLACES the typed map before the overlay merges, so the effective pre-overlay set is `extraOptions.mcpServers ?? config.mcpServers` — with `extraOptions.mcpServers = {}` a typed `foo` is NOT occupied) plus `INJECTED_SERVER_NAMES`; stamp `record.dynamicTools` (the field exists since Task 7); **the `initialize` result (or `server/status` — whichever carries server capabilities today; find it) gains a `dynamicTools: true` capability marker** (r7 finding 7 — the F9 lesson: an old server SILENTLY strips the optional field and starts the thread toolless; a new client must be able to detect the downgrade before declaring; the spec's wire section gains the marker and the client guidance); regenerate the artifact — **assert `stable.results["tool/callResult"]` is present in the schemaGen expectations**.
