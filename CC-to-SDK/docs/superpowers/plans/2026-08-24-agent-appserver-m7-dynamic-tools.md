...
- **Names:** `toolName` regex `/^[A-Za-z][A-Za-z0-9_-]{0,63}$/`; namespace `dyn` RESERVED; duplicate namespace names across the declaration set refused. Collisions are checked for **every name `overlayServerNames` would produce — `dyn` included when bare functions exist** — against the EFFECTIVE occupied set: `Object.keys(config.mcpServers ?? {})` ∪ `Object.keys((config.extraOptions?.mcpServers as object) ?? {})` ∪ `INJECTED_SERVER_NAMES` (the in-process servers `Session` adds AFTER `resolveOptions` — enumerate them at the Session build site and export the const beside them; `cc-context` and `cc-compact` at minimum). Function names are checked against `NATIVE_TOOL_NAMES` — the COMPLETE measured native catalog (the [[sdk-native-tool-catalog]] probe's ~37-tool list, pinned with a provenance comment naming the probe), NOT `items/types.ts`'s display classifier, whose `switch` is a subset that omits `TaskCreate`, `AskUserQuestion`, `Workflow`, and others. Every declaration refusal is `-32602` naming the offender.
...
export const NATIVE_TOOL_NAMES: readonly string[];  // the COMPLETE measured catalog (sdk-native-tool-catalog probe), provenance comment required — NOT the items/types.ts display subset
...
Declaration wiring at `thread/start`/`thread/resume` (server.ts:423-460 + the resume handler): after `safeParse`, `validateDeclarations(specs, occupiedNames)` where `occupiedNames = [...Object.keys(cfg?.mcpServers ?? {}), ...Object.keys((cfg?.extraOptions as {mcpServers?: object} | undefined)?.mcpServers ?? {}), ...INJECTED_SERVER_NAMES]`; failure → `-32602`; success → pass specs to `createThread`/`startThread`, stamp `record.dynamicTools` in the record literals (server.ts:729/:786). The DynamicCalls registry mints beside `makeDecisions` (:943 pattern), releases in `closeRecord` (:886 — `teardown()` + map delete), resets in `threadReopen`, and resets in `swapEngine` **immediately after the epoch bump and BEFORE the `session.dispose()` await** (r2 finding 6: dispose can wait on the parked handler's promise — reset-after-dispose is a circular wait; the ordering is part of the contract, state it in a comment at the call site), tears down in the server shutdown path beside decisions.
...
- [ ] **Step 1: Failing wire rows** — `test/unit/appserver/dynamic-tools.test.ts` (boot idiom from `decisions.test.ts`; park via `srv.parkToolCall`):
  ...
  - lifecycle: abort signal; `thread/close` with a park OPEN — **the fake session's `dispose()` resolves only after the park settles** (the deadlock-shaped fake: `dispose: () => pendingSettled`), assert close COMPLETES and the park resolved cancelled; **the SAME deadlock-shaped fake on the rewind swap** (r2 finding 6: dispose waits on the park; the swap completes only if reset precedes the dispose await) and on `thread/clear`; server `close()` (shutdown row); `thread/reopen` reset; old-generation reuse: park → rewind swap (epoch bump) → new park → answer the OLD callId → `-33002`, the NEW park untouched; **stale-generation caller: `srv.parkToolCall(threadId, oldEpoch, …)` after a swap resolves cancelled immediately with no registry entry and no notification** (r2 finding 1); a park attempted with no active turn resolves cancelled immediately;
...
- Modify: `src/config/types.ts` (`dynamicToolServers?: Record<string, unknown>` on the config type `resolveOptions` reads), `src/config/resolveOptions.ts`, `src/appserver/server.ts:722-733` + `:757-799`, `src/appserver/rewind.ts` (`swapBaseConfig` + `defaultReopenFactory` + `defaultResumeAtFactory`), **`src/appserver/settingsOps.ts` (`defaultFreshFactory` — `thread/clear`'s factory lives HERE, not in rewind.ts; r2 finding 5)**, `src/appserver/mcp.ts:115-158` (the refusal)
...
- [ ] **Step 5: Commit** — `git add src/appserver/dynamicServers.ts src/config/types.ts src/config/resolveOptions.ts src/appserver/server.ts src/appserver/rewind.ts src/appserver/settingsOps.ts src/appserver/mcp.ts test/unit/appserver/dynamic-tools.test.ts <the resolveOptions suite file you extended> && git commit -m "feat(appserver): dynamic-tool servers — transient overlay, merge-last, set refused on declaring threads"`
...
- [ ] **Step 1: Exchange rows** (`dynamic-tools-exchange.test.ts`) — TWO layers:
  - *Instance-direct* (fidelity): `buildDynamicServers` with a synthetic park; `client.listTools()` advertises every declared property/required/bounds/description (structural compare); **the initialize result carries the namespace description as `instructions`** (r2 finding 7's assertion); passthrough extras reach the park's `arguments`; `deferLoading` mapping — measure which surface carries `_meta["anthropic/alwaysLoad"]` (tools/list `_meta` or the built definitions) and pin `true` for omitted/false, absent-or-false for `deferLoading:true`; **the handler's `extra.signal` reaches the park callback** (abort the client call, observe the cancellation).
