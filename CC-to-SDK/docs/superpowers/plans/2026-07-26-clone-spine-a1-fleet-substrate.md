...
- **2026-07-26, from the Task 7 review — two breaches of the plan's own central invariant.** "Only terminal state is recorded" was violated on two reachable paths. `stop()` called with no argument ran `finalizeRoster` with whatever the in-memory state happened to be, so a host stopped before or without a turn wrote `state: "working"` stamped with an `endedAt` — a row that looks ended but is not terminal, which a poller waits on forever. And the first completed turn finalized unconditionally, so a host that takes a second turn is recorded terminal while still alive. `syncRoster` now finalizes only terminal states, and the turn-completion finalize is **gated on `kind === "bg"`**: for a background worker the turn's completion *is* the terminal event, and recording it there means a host that dies after the turn but before `stop()` still reports `done` rather than relying on liveness reaping; an interactive host stays live across turns and finalizes only at `stop()`. `start()` also gained a failure path — it previously left a permanent `working` row and an undisposed session if `openSession` or `listen()` threw, and nothing in the codebase reaps that.
  *Recorded for Task 9 and 11:* `stop()` awaits `session.dispose()`, which itself waits for an in-flight turn to finish, and only then closes the socket. So a socket `stop` op does not return until the current turn ends naturally. Task 11's caller already treats a missing acknowledgement as "already gone", which is the right behavior — but nothing here interrupts a running turn.
