...
- ~~GC (L495694): an edit that removes a chip's text from the buffer deletes its map entry~~ **CORRECTED (2026-08-05, t4 re-review — bundle wins):** upstream's live GC (L495715–728) is image/audio-filtered; TEXT entries are never pruned live — the map survives until submit, where the outgoing map is rebuilt from the ids actually present in the text (L536788–792), making stale entries inert. Live GC for text chips produced payload loss at every park site (history browse, kill-ring yank, ctrl+s stash). Ship upstream's semantic: no live GC; `substituteChips` at submit expands only ids present in the text. Undo resurrects chip + map (Task 1's entries carry the map).
