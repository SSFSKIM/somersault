...
- Observation: implementation review corrections, including the expanded ownership and verification fixes, are grouped by evidence family: sed execute and redirect safety; sandbox deny/ask; remote/command-specific too-complex handling; required effect ports; inherited assignment safety; clamp literal/wildcard/prefix/xargs/Bash(*)/escaped parsing; multi-cd removal and candidate cwd; awaiting async safety; resolved leading-cd path checks; empty suggestion fallback; rule dedup keys; classifier-prefix rules; AbortError; exact env-prefix allowlist; Windows cwd normalization; cwd forwarded to git/cd-git checks; See clamp redirection projection; and the pnn assignment traversal boundary.
  Evidence: `reforge/build/c13b-fix-round2-helper-oracle-final.log` reports 1,007 checks and 62 named controls; `reforge/build/c13b-fix-round2-qualified-coverage-final.log` reports 1,083/3,276 outcomes across 1,693 sites; `reforge/build/c13b-final-permissions.log` reports 2,508 comparisons and 49 controls.
- Observation: aggregate contract credit requires the complete target, input, effect schedule, and configuration to match a parity-qualified invocation. Removing private and schedule-mismatched calls lowers measured execution instead of preserving an unsupported total. The shared partitions remain 71 helper, 15 named semantic, 29 pipe, two pre-validator aggregate, four mode, 30 security-regression, and 2,191 parser cases. The twelve special residual outcomes remain four producer invariants, three control-flow impossibilities, three caller-outside-domain cases, and two resource-sensitive cases; 1,902 open-input, 276 validator-domain, and three port-state outcomes remain explicit gaps.
  Evidence: `reforge/build/c13b-fix-round2-qualified-coverage-final.log` reports 1,083/3,276 outcomes over 1,693 generated sites and names every residual category.
- Observation: twelve graph-side captures advertised as pure helpers across nineteen sites were still executing pinned engine logic. The owned replacements now compare the exact `_Tt`, `ql`, `Q`, `te`, `Bn`, `DU`, `Rpn`, `pwe`, `kze`, `S_`, `YTe`, and `nf` declarations behaviorally. This oracle found two additional transcription defects: `isolatePeerMachines` is not classifier-routed, and `YTe` emits `Read` rules. `qN` is not pure because it calls `statSync`, while the path aliases are direct `node:path` dependencies.
  Evidence: `reforge/build/c13b-fix-round2-pure-helper-behavior-red.log` preserves the two valid semantic REDs, `reforge/build/c13b-fix-round2-helper-oracle-final.log` reports 1,007/62 after complete reason-mode and Windows/UNC partitions, and `reforge/build/c13b-fix-round2-adapters-final.log` rejects every graph-side pure-helper site.
...
- Observation: aggregate parity and aggregate contract coverage cannot safely run concurrently. The coverage path regenerates instrumented module state, so the authoritative parity total is the sequential 1,007 measured before coverage takes that state.
  Evidence: `reforge/build/c13b-fix-round2-helper-oracle-final.log` and `reforge/build/c13b-fix-round2-qualified-coverage-final.log` are the sequential durable outputs.
- Observation: the three aggregate adapters retain eight primitive assertions, and the 25 child adapters add 23 primitive capture sites, including the asserted `node:path` separator. Reference-typed Sets and RegExps require structural equality, not `Object.is`; the expanded suite proves healthy independent copies and perturbs every site before delegation.
  Evidence: `reforge/build/c13b-fix-round2-adapters-final.log` reports 262 checks; `reforge/build/c13b-fix-round2-captures-final.log` reports 329 derivation perturbations.
...
- Decision: the pinned-byte oracle binds upstream bodies to upstream helpers and exercises owned adapters, while table controls perturb one named entry at a time.
  ...
  Date/Author: 2026-09-07 / C13b implementer.
- Decision: run aggregate parity to completion before aggregate coverage, and treat 1,007 as the authoritative focused parity count.
