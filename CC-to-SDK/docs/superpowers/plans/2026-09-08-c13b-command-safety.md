...
- Observation: implementation review corrections, including the expanded ownership and verification fixes, are grouped by evidence family: sed execute and redirect safety; sandbox deny/ask; remote/command-specific too-complex handling; required effect ports; inherited assignment safety; clamp literal/wildcard/prefix/xargs/Bash(*)/escaped parsing; multi-cd removal and candidate cwd; awaiting async safety; resolved leading-cd path checks; empty suggestion fallback; rule dedup keys; classifier-prefix rules; AbortError; exact env-prefix allowlist; Windows cwd normalization; cwd forwarded to git/cd-git checks; See clamp redirection projection; and the pnn assignment traversal boundary.
  Evidence: `reforge/build/c13b-fix-final-parity.log` reports 822 checks and 57 named controls; `reforge/build/c13b-fix-final-aggregate-coverage.log` reports 1,103/3,087 outcomes across 1,592 sites; `reforge/build/c13b-final-permissions.log` reports 2,508 comparisons and 49 controls.
- Observation: the helper-corpus expansion adds 103 newly covered non-validator outcomes plus one validator outcome; it does not claim that every reachable helper outcome is closed. Its shared partitions are 71 helper, 15 named semantic, 29 pipe, two pre-validator aggregate, four mode, 30 security-regression, and 2,191 parser cases. The twelve special residual outcomes are four producer invariants, three control-flow impossibilities, three caller-outside-domain cases, and two resource-sensitive cases.
  Evidence: `reforge/build/c13b-fix-final-aggregate-coverage.log` reports 1,103/3,087 outcomes over 1,592 generated sites and names every residual category.
- Observation: aggregate parity and aggregate contract coverage cannot safely run concurrently. The coverage path regenerates instrumented module state, so the authoritative parity total is the sequential 822 measured before coverage takes that state.
...
- Observation: the three aggregate adapters retain eight primitive assertions, and the 25 child adapters add 22 primitive capture sites. Reference-typed Sets and RegExps require structural equality, not `Object.is`; the expanded suite proves healthy independent copies and perturbs every site before delegation.
  Evidence: `reforge/build/c13b-fix-decision-adapter-controls.log` reports 255 checks; `reforge/build/c13b-fix-final-captures-after-perturb-fix.log` reports 329 derivation perturbations.
...
- Decision: the pinned-byte oracle binds upstream bodies to upstream helpers and exercises owned adapters, while table controls perturb one named entry at a time.
  ...
  Date/Author: 2026-09-07 / C13b implementer.
- Decision: run aggregate parity to completion before aggregate coverage, and treat 822 as the authoritative focused parity count.
...
The expanded implementation owns the child command decisions rather than merely delegating them through graph ports. The first valid `bash-dangerous-removal` take now closes the `drn`/`Fy` recording edge safely, and its targeted `Fy`-blind mutant fails on the preserved reason. Direct pinned-byte contracts cover exact decisions, duplicate tie-breaking, insertion-ordered `Map` behavior, and `subcommandResults`; the occurrence-sensitive `jrn` recording edge remains pending. Focused attestation accounts for all 12,042 outcomes: 2,075 by corpus replay, 6,393 by the producing module's differential driver, and 3,574 by reviewed exclusion. The parent still owns independent review and the full strangler gate, so this plan does not claim final C13b or campaign closure.
...
Revision note (2026-09-08): expanded the command-admission boundary to 30 runtime splices and eleven folded production declarations, added structural and perturbation controls for all child adapters, classified all 25 child twins with clean-start replay, recorded the first valid `bash-dangerous-removal` take, and proved an `Fy`-blind mutant changes the reason to the generic multi-`cd` refusal. Final focused attestation accounts for 2,075 corpus + 6,393 contract + 3,574 reviewed = 12,042 outcomes across 6,189 sites. The `jrn` occurrence-sensitive recording edge, parent review, and full gate remain pending.
