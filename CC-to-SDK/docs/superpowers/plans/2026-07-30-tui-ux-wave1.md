...
- [ ] **Step 2: Update `tui-ux.md`.** Historical Wave-1 checklist; F0 superseded its Ctrl-Z detach
  divergence with Ctrl-Z suspend and `/detach`. Add/refresh rows for: Shift+Tab mode cycle (converged),
  Ctrl-L clear-input (converged), Ctrl-J newline, Ctrl-_ undo, Ctrl-S stash, Ctrl-T todo toggle, Ctrl-X
  Ctrl-E external editor, and the remaining intentional divergences (`cmd+k` screen-clear unreachable →
  `/clear`; Ctrl-B background vs upstream's task:background). Note ctrl+o/ctrl+r/ctrl+x ctrl+k deferred to Wave 2.
