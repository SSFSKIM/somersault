...
it("Ctrl-O opens the transcript pager, gates the app keys, and Ctrl-O again closes it", async () => {
  ...
  await tick();
  expect(lastFrame()).not.toContain("of 0");             // pager gone, composer back
});
```
...
3. The app-level `useInput` gains a gate at the very top and the Ctrl-O arm (the pager owns every key while open — its own exit keys close it; only the Global Ctrl-O toggle stays here, matching the bundle where `app:toggleTranscript` is Global):
...
useInput((input, key) => {
  ...
  /* …existing ctrl+z / ctrl+c / ctrl+b arms unchanged… */
});
```
...
- Modify: `src/tui/ChatApp.tsx` (Ctrl-R arm + overlay-chain slot)
- Modify: `src/tui/ShortcutsOverlay.tsx` (one ROWS entry)
- Test: `test/tui/historySearchOverlay.test.tsx` (new), `test/tui/chat.test.tsx` (extend)
...
In `test/tui/chat.test.tsx` add wiring tests (existing app harness; its fake session's `getSessionMessages` dep already exists — extend the deps with `listHistorySessions` where the harness passes deps):

```ts
it("Ctrl-R opens history search; Esc accepts the top entry into the composer", async () => {
  /* fake deps: getSessionMessages resolves one prompt row {type:"user", uuid:"u1", message:{content:"redo the build"}};
     listHistorySessions resolves [{ sessionId: "s1", summary: "", lastModified: 1 }] */
...
2. App-level `useInput`, after the Ctrl-O open arm (still inside the non-pager branch): `if (key.ctrl && input === "r") { openHistorySearch(); disarm(); return; }   // CC history:search (Global)`
3. Overlay chain, immediately after the pager arm:
...
3. Record the Wave-2 divergences verbatim from this plan's Global Constraints: pager-as-bordered-overlay (Static remount lesson), `ctrl+e` toggleShowAll deferred, `home`/`end` unreachable (g/G equivalents), `local_agent` output not tailed, wrapped lines can overrun the pager window (conservative height budget).
...
The driver types text lines and sends Enter separately (`scripts/drive-repl.py` docstring); raw control bytes are embeddable in arguments (`\x0f`=Ctrl-O, `\x12`=Ctrl-R, `\x18`=Ctrl-X, `\x0b`=Ctrl-K). Run keyed, from `harness/`:
