...
3. The app-level `useInput` gains a gate at the very top and the Ctrl-O arm. Two rules the review pinned: **Ctrl-Z (detach) moves ABOVE the gate** — detach stays reachable even under overlays, consistent with the file's own "detach ≠ deny" stance — and while the pager is open every other app arm is gated (the pager owns the keys; its own ctrl+c = transcript:exit per the bundle, so the app's Ctrl-C exit-arm must not also fire):
