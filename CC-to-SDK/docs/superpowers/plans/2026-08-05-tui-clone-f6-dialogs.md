...
| 4 | ~~DG31: empty keeps the dialog open~~ **CORRECTED (T9 review):** an empty keep-planning submit routes to the Select's `onCancel` (`sYf` L500713 sets no flag; `RLe` L397113-118) → `xnl` → plain deny, same as Esc (L500995). The `return null` at L500732-736 is the `"no"` arm, reachable only with pasted images — dead here. Non-empty → `plan_reject` with the feedback | L500713, L397113-118, L500995 |
...
4. *A plan approval is titled `Ready to code?`; choosing `No, keep planning` and submitting empty feedback rejects with no feedback, exactly as Esc does.* (Rewritten per the spec Revision Note 2026-08-05 — the census's "stays open" read `lYf`'s null-return, which sits on an arm an empty submit cannot reach: `sYf` L500713 sets no `allowEmptySubmitToCancel`, so `RLe` L397113-118 routes empty to `onCancel` → `xnl` L500995 deny.)
