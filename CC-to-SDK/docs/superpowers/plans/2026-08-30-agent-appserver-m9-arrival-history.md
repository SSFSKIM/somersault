...
- **D1 — no wire break.** `thread/read`'s published cursor pattern `^\d+:\d+$` must not change; `schema/json/stable/appserver.json` (path relative to `harness/`) may change only by *adding optional response fields* (`origin` on user items, `arrivals` on read/search replies). Run **`npm run emit-schema`** after schema-source edits and commit the regenerated artifact in the same commit; diff the artifact and confirm the cursor pattern line is byte-identical.
...
- All commands run from `CC-to-SDK/harness/`. Unit suite: `npx vitest run test/unit/<path>`. Typecheck: `npx tsc --noEmit`. **One exception — the capability gate `(cd .. && node scripts/drift-check.mjs)` runs from `CC-to-SDK/`**, one directory up, because the script lives at `CC-to-SDK/scripts/drift-check.mjs`; invoked from `harness/` it fails `MODULE_NOT_FOUND`. Never edit it. Every gate line below spells the same `cd ..` for that one command.
...
- **Seeding runs when the session id is actually known** (spec rev 8.1): a record admitted *with* a `sessionId` (attach/resume) seeds at install; a record *without* one — which includes **fork admission**, which deliberately leaves `record.sessionId` undefined over copied history — seeds **at the init frame**, against whatever id init reveals. Never ground confirmed-empty merely because there is no id yet. The seed itself: fire `getSessionMessages(sessionId)` (the effective reader from deps); on resolve, ground per the **overlap rule + ambiguity floor** (spec verbatim): earliest buffered frame uuid found in the seed → ground on the seed row before it; no overlap and the seed returned rows → any buffered *arrivals* are entries with `ambiguous: true` (persisted, counted, never placed — this is criteria 12 and 15's shared outcome, not two rules); no overlap and empty seed → ground confirmed-empty (the reader maps read failures to `[]`; the spec records that as a limit — do not try to distinguish here); then replay `seeding.frames` in order to advance the anchor, then flush `seeding.arrivals` in order (persist-then-broadcast each). The pre-init crash window stays the stated limit.
- `uninstallPeerInbound` clears the seeding state alongside its existing cleanup.

- [ ] **Step 1: Write the failing tests.** Use the same fake-session harness `test/unit/appserver/peer-inbound.test.ts` already uses (read it first; reuse its `installPeerInbound` fixture pattern). Cover, one `it` each, citing the criterion: (6) one entry per `thread/peerMessage`, ids equal as sets; (7) every non-null anchor names a row the injected reader returns, `prevUuid` its predecessor, `fp.hash === contentHash16` of that row's text; (8) a folded arrival (frame while adopted turn runs) is logged; (10) both halves: a successful arrival's `append` is observed to complete *before* its broadcast (spy on both, assert call order), and append-throw → notification still broadcast + `isDegraded` true (inject a store whose `append` throws once, marker path left writable); (11) restart ordering — two same-anchor arrivals, new store instance between them, seqs increase; (12) delayed seed (reader returns a pending promise) + immediate arrival → persisted only after resolve, never anchored null: `ambiguous: true` when no live frame grounded it, anchored to the live frame when one did (two `it`s — this and criterion 15 are one state machine); (12b, fork shape) record admitted WITHOUT a sessionId whose init reveals an id with existing rows → seeds at init, grounds on the copied tail, and an arrival then anchors there rather than at the top; (14) the four overlap shapes — seed-behind, seed-ahead, partial overlap, seed-tail-equals-buffer-head — each frame anchoring exactly once; (15) buffer holds only an arrival while the seed returns unseen rows → entry has `ambiguous: true`, is counted in `counts().logged`, and Task 4's resolver (once it exists) never places it — here assert the flag and the count; (16) fingerprint recording fixtures — differing-parent duplicate shape, a live frame with NO `timestamp` (recorded fp omits it), and a `timestamp`-carrying frame (recorded fp includes it) — assert the *recorded* anchor fields exactly.
...
- [ ] **Step 3b: Criterion 9, explicitly.** One more test in the new file: with the store populated, drive the server fixture's `thread/read` and assert its reply is deep-equal to the empty-store reply — Stage B changes nothing a reader can see, which is what makes it verifiable alone.
...
- Modify: `src/appserver/items/types.ts` (one additive field), `src/appserver/items/mapper.ts` (export one origin-capable constructor, `arrivalItem(text, id, origin)`, beside `userItem`), `src/appserver/peerInbound.ts` (the live queue keeps `origin`; `drainArrivals` emits `arrivalItem` — **criterion 20 is three-path parity, not a projector feature**), `src/appserver/items/replay.ts` (a cold peer row's item carries `peerArrival(f).origin`)
...
- [ ] **Step 1: Write the failing tests.**
  ...
  - Arrival injection: anchored entry emits immediately after its row's items; an item opening at the anchor row but completing later emits after the arrival (assert with a tool_use at the anchor and its tool_result later); `origin` present on the arrival item; text is `entry.text` verbatim (feed a collapsed-batch-shaped text and assert both messages present).
  - **Three-path parity (criterion 20):** for one arrival, deep-compare `{ id, text, origin }` across (a) the live drained item (`drainArrivals` via the peerInbound fixture), (b) the cold replayed item (`itemsFromTranscript` over the persisted peer row), and (c) the projected item — all three equal.
...
- Modify: `src/appserver/schema/threads.ts` **and `src/appserver/schema/index.ts`**: `thread/read` currently registers only a *params* schema — define a complete `threadReadResult` (data items with the optional `origin`, `nextCursor`, optional `arrivals`) and register it; then regenerate via **`npm run emit-schema`** (NOT `write-schema`, which does not exist) and commit `schema/json/stable/appserver.json` (path relative to `harness/`)
...
- Produces (`arrivalsReply.ts`):
  ...
  ```
  Also export **`anchorMatchesRow(anchor: ArrivalAnchor, row: unknown, predecessor: unknown | null | "unknown"): boolean`** — the single anchor-match predicate; `resolveArrivals` is built on it and **Task 5 imports it** rather than re-implementing matching.
...
- [ ] **Step 4: Run** — `npx vitest run test/unit/appserver/subscribe-arrivals.test.ts test/unit/appserver/subscribe.test.ts test/unit/appserver/items` → all PASS. `npx tsc --noEmit`, `(cd .. && node scripts/drift-check.mjs)` → clean.
...
**Files:**
- Modify: `src/appserver/search.ts`, **`src/appserver/searchScan.ts`** (the module that actually owns the opaque occurrence cursor — its `OccCursor` shape, encoder and decoder live there, not in `search.ts`)
- Modify: `src/appserver/schema/search.ts` (optional `arrivals` on the reply) + `npm run emit-schema` + commit the artifact
- Test: `test/unit/appserver/search-arrivals.test.ts`; Modify: **`test/unit/appserver/search-scan.test.ts`** (the codec's own tests)

**Interfaces:**
- Consumes: Tasks 1/2/4 — `effectiveArrivalStore`, and **`anchorMatchesRow` from `arrivalsReply.ts`** (produced by Task 4; do not re-implement matching).
- **Cursor compatibility is mandatory:** the legacy encoded shape (`{s,r,c,q,g}`-style fields — read `searchScan.ts` for the real names) must decode as the *row* phase; a cursor minted before M9 mid-walk resumes correctly after the upgrade. Add that exact test to `search-scan.test.ts`.
- **Left edge and early exits:** whenever a scan window starts above row 0, fetch and peel one predecessor row (same pattern as Task 4's lookbehind) so a first-row anchor can verify `prevUuid`; and run the arrival-scan phase for **every** row — *outside* the existing `continue`s for null/oversized `rowSearchText` (a system row has null search text and is still a valid anchor). Preserve the existing cold-session rule: `readCursor: null` when no epoch exists, for arrival occurrences too.
...
- [ ] **Step 1: Write the failing tests**: (27) a retained arrival's text found, and **feeding its `readCursor` to `thread/read` returns a page whose items contain that arrival**; after filling past the cap, the evicted arrival's text is NOT found and the reply's `arrivals.dropped > 0`; null-anchored arrival findable when a first row exists; a **cold session's** arrival occurrence has `readCursor: null`; an arrival anchored to a **null-search-text (system) row** is still scanned; an arrival anchored at a **resumed window's first row** is found (the peel); (28) two same-anchor arrivals both matching, `limit:1` walk visits both exactly once; two matches inside ONE arrival at `limit:1` resume on the entry-local offset; a **pre-M9-shaped legacy cursor** resumes as the row phase.
- [ ] **Step 2: Run to verify failure.**
- [ ] **Step 3: Implement**; keep `thread/search` (the store-wide sibling) untouched.
- [ ] **Step 4: Run** — `npx vitest run test/unit/appserver/search-arrivals.test.ts test/unit/appserver/search.test.ts` → PASS. `npx tsc --noEmit`, `(cd .. && node scripts/drift-check.mjs)` → clean.
...
- [ ] **Step 1: Write the tests** (these run against the assembled server fixture used by `test/unit/appserver/fr-*.test.ts` — read one for the harness pattern):
  ...
  - (13) **degraded survives restart when the marker can be written:** inject a store whose `append` throws exactly once while its marker path stays writable (wrap `fsArrivalStore` with a one-shot failing `append` that still calls the real `markDegraded`) → `arrivals: null` on reads; rebuild the server over the same root → still `arrivals: null`. A *fully* unwritable dir asserts only the in-process latch (the spec's stated limit — do NOT expect the marker to survive that).
  - (23) **count equality:** across a keyed-style fake run, `arrivals.logged` on the final page equals the number of `thread/peerMessage` notifications observed.
...
- [ ] **Step 2–4: Red → implement any glue the tests expose → green**, then the full gate: `npx vitest run test/unit` (entire unit suite), `npx tsc --noEmit`, `(cd .. && node scripts/drift-check.mjs)`.
...
- [ ] **Step 1:** Full gates from `harness/`: `npx vitest run test/unit` → expect **all green** (record counts); `npx tsc --noEmit` → exit 0; `(cd .. && node scripts/drift-check.mjs)` → exit 0.
- [ ] **Step 2:** Verify the traceability table below by running each named test and confirming it asserts what its criterion says. A row whose test is missing or vacuous is a FAIL for the *owning* task — dispatch a fix there, then re-verify. Write the completed table (with pass marks) into the task report.

| Crit | Owning task · test |
| --- | --- |
| 6 | T2 · peer-inbound-log: one entry per announcement, ids equal as sets |
| 7 | T2 · peer-inbound-log: anchor names an existing row, prevUuid its predecessor, fp.hash matches |
| 8 | T2 · peer-inbound-log: folded arrival logged |
| 9 | T2 · peer-inbound-log (Step 3b): populated store, read reply deep-equal to empty-store reply |
| 10 | T2 · peer-inbound-log: append-before-broadcast call order; throw → broadcast + degraded |
| 11 | T1 · arrival-log: nextSeq across re-open; T2: restart ordering end-to-end |
| 12 | T2 · peer-inbound-log: delayed-seed arrival never null (ambiguous or live-frame-anchored); fork shape 12b |
| 13 | T6 · arrivals-clear-degraded: one-shot failure durable across rebuild; fully-unwritable = in-process only |
| 14 | T2 · peer-inbound-log: four overlap shapes |
| 15 | T2 · peer-inbound-log: arrival-only buffer + unseen seed rows → ambiguous, counted, unplaced |
| 16 | T2 (recording) + T4 (resolution: timestamp-absent; indistinguishable-duplicate output-equivalence) |
| 17 | T1 · arrival-log: eviction totals + counted-victim recovery never under-reports |
| 18 | T3 · project: parity property over the corpus |
| 19 | T7 · live LEG 2 flipped: arrival item precedes the answer |
| 20 | T3 · project: three-path {id,text,origin} parity; T7 live: collapsed batch carries both texts |
| 21 | T4 · subscribe-arrivals: cursor pattern/behaviour unchanged; artifact diff pins the pattern line |
| 22 | T4 · subscribe-arrivals: the full limit:1 edge set incl. round-5 walk + round-6 left edge |
| 23 | T4 · subscribe-arrivals: field on all five paths; T6: logged equals notification count |
| 24 | T4 · subscribe-arrivals: withhold on row/hash/predecessor change, logged exceeds marked items |
| 25 | T6 · arrivals-clear-degraded: clear then resume |
| 26 | T4 · subscribe-arrivals: reader-without-store → no merge, no arrivals key |
| 27 | T5 · search-arrivals: found + readCursor page contains the arrival; evicted → not found + dropped>0; cold → null |
| 28 | T5 · search-arrivals + search-scan: same-anchor pair, intra-entry offset, legacy cursor as row phase |
