...
- **`cc-harness-console`** — cycles all six modes incl. `auto` (`useDaemon.ts:5`), but switching a *running* non-auto session to `auto` doesn't re-force its model, so it silently degrades (the daemon only forces the model at *spawn*). **[Corrected 2026-08-06 by probe 99: the RUNTIME setter is REFUSED on an unsupported model — "Cannot set permission mode to auto: auto mode unavailable for this model" — not silently degraded. The model swap is still required; without it the mode change simply does not happen.]**
