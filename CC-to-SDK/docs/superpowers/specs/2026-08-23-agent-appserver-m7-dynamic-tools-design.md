...
**The final whole-branch codex campaign closed clean in five rounds (2026-08-24, base `781683b4cf`, trajectory 1→1(+2 rejected)→3→2→0).** Seven findings were fixed, each with rows and sabotage proofs: the malformed-media error note reflected an unbounded client MIME past the result cap (fixed twice over — truncation at the interpolation site AND a structural clamp inside `errorResult` itself); `.int()` silently narrowed the advertised integer domain to JS safe integers (2^53 was advertised-valid, refused at CallTool — replaced with a `Number.isInteger` refinement, ±2^53 pinned); description bounds counted UTF-16 units while the artifact published code-point `maxLength` (aligned via a code-point refine carrying `.meta({maxLength})` — zod v4 merges registry metadata into `toJSONSchema` verbatim, so the artifact stayed byte-identical); duplicate `required` entries and duplicate `enum` members were advertised verbatim and AJV rejects both at compile time, killing the whole `tools/list` (both now refuse at declaration); and a bare tool with `deferLoading: true` diverged from the canonical Codex admission (`thread_processor.rs` refuses “deferred dynamic tool must include a namespace” — this server now answers the same words). One finding became a published bound instead of code: multiple subscribers that each execute a non-idempotent tool race — first result settles, later answers get `-33002`, executing clients coordinate among themselves (the decisions surface's own property). Two findings were rejected as misapplied Rust-repo rules (`AGENTS.md`'s context-item token caps and 800-line change guidance govern `codex-rs/`, not this TypeScript harness).
