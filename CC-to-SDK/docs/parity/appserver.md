...
Ten methods answer for the SERVER rather than mirroring a seam in one of the four sources above, so
...
eight of the ten have none, so those eight read `N/A` there — the adoption pair included, whose SUBJECT
is a fleet session but whose caller names no thread at all, the workspace pair, which a client roots on
a thread's tree by passing `threadView.cwd` as a plain path, and M5's `config/read`, whose subject is a set
...
M5's `config/read` (`appserver/configDomain.ts`) is a **tenth**, and the first of the config domain: the settings
files it reads are this machine's, named by no thread — `fs/read`'s reason exactly. Its row is in the table below.

M4's `review/start` is an **eleventh** method of this kind — nothing walks it either, for the same reason
...
all rowed above — plus the 10 server-origin rows, which no walker produces, plus M4's 2 review rows, for
**91 rows** in all. (Recounted off the tables at M3 Task 12, which read "3 / 82" from the M2b close-out
...
agreed at 88 there, at 90 at M4 Task 9, and at 91 here (M5 Task 2's `config/read`). **The gate would not have caught a miscount at this landing**, which is
...
What those two lines say at this sweep (**M5 Task 2, 2026-08-18** — restated per landing, never trusted
between them): **91 rows, 89 of them shipped** (2 of those `shipped(M4)`, 1 `shipped(M5)`), the remaining two being the
...
files being the machine's rather than a conversation's. The rows that remain
...
the live surface (N registered methods)"), it read 51 at the M2b close-out, 58 at M3's, 59 at M4's and 60 at M5 Task 2's `config/read`, and
the run is
