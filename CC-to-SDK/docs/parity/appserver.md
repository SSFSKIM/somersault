...
| `getSessionMessages` | sessions/index.ts | `thread/read` | both | shipped(M1) — M2a re-cursored it (absolute row offset, 500 clamp); **M9 merges an arrival log into the reply, so `thread/read` no longer returns only what this reader returns.** The CLI persists an inbound peer message as an `isMeta` row and `getSessionMessages` drops every `isMeta` row with or without `includeSystemMessages`, so history held an answer whose question was nowhere in it — the absence M8's LEG 2 asserted, and the presence it asserts now. The server therefore LOGS the arrival at the moment it observes it (`peer/arrivalLog.ts`: one file per entry under the session id, append-only, capped; anchored to the last **user/assistant** frame it had seen — the predicate both readers of the transcript actually use — carrying that row's `prevUuid` and a SHA-256/16 hash of its content, so a rewrite-born duplicate at another position cannot claim the anchor), and `appserver/items/project.ts` injects each resolved entry as a `userMessage` ITEM at its anchor row on every read. **Items, not rows:** an arrival RIDES the row it followed rather than occupying one, so no coordinate moved — the published cursor is still `^\d+:\d+$` over raw row offsets, still generation-qualified, and byte-identical in the generated artifact (D1); the only wire change is two additive optional RESPONSE members. The item carries the entry's text VERBATIM — including the two messages of a batch the engine collapsed into one frame, which re-deriving the text through `peerArrival` at read time would re-lose — plus `origin`, the attribution the kernel vouched for, which is what lets a client tell an arrival item from an ordinary prompt and count them. Every merge-enabled reply carries `arrivals: {logged, dropped}` on all five paths (cursorless, normal page, last resort, exhausted cursor, no session): `logged` is the PRE-eviction total, so a client holding both the count and the marked items computes for itself the withheld count this server refuses to fabricate; `null` where the store latched degraded; and ABSENT — not `0` — where an embedder overrode `getSessionMessages` without supplying a store, since no merge is performed over a transcript this machine does not own. **Withhold, never misplace:** an entry whose anchor row is gone, whose fingerprint moved, whose predecessor changed (the rebound-duplicate shape), or that was flushed ambiguous does not render at all, no other item's position shifts, and the omission stays visible because `logged` exceeds the marked items returned. The `anchor: null` sentinel — "before every row the seed returned" — sits outside the bisection and renders only on a page that both includes row 0 and discards nothing, so the head of history lands on the walk's terminal page rather than on a cursorless first page that is really the newest items. **Stated limits, in the spec and here:** an arrival that lands before the engine reveals a session id is announced and never logged (no scope to key an entry by); a store-write failure degrades loudly and durably wherever the marker can still be written, but a directory that cannot be written AT ALL latches in memory only; the reader maps its own filesystem failures to an empty transcript, so a read that fails resolves every anchor to nothing; and per-message identity inside a collapsed batch does not exist in the data at all — one frame, one entry, both texts under one uuid |
...
M8's `peer/list` and `peer/send` join the table below rather than a section of their own, and the
reason is the distinction the review and dynamic-tools pairs turn on: `peer/messageStatus` is not the
announcement channel of a method, it is one REQUEST's delayed completion, so pairing it off into its own
section would claim a unity the archive notifications already show this table can carry. Nothing walks
either method: the peers they name are other programs' sessions, read out of the engine's own registry
directory, and neither the host wire nor `interface Query` has a notion of a session this server does not
own. Both score `N/A` for origin, `fleet/list`'s and the config trio's reason — they name no thread at
all — and `peer/send`'s optional `fromThreadId` does not change that: it is ATTRIBUTION, not subject,
which is exactly why a fleet-origin thread is refused there instead of the method being scored `both`.
`peer/messageStatus` is the third row here that is not a method, after the archive pair, and the one
whose scope is neither a thread nor the server but a single CONNECTION: a receipt belongs to whoever
sent the message, and a client that has gone away is owed nothing.
...
| `peer/send` | appserver/peerDomain.ts | `peer/send` | N/A | shipped(M8) — writes one enveloped frame to a peer's inbox (behind an `auth` frame carrying that peer's own vouching token, when its key file is readable) and **reports nothing more**: `delivered` is a literal `false`, because the CLI tells a sender nothing on the success path (measured — a DELIVERED message produces no receipt at all; a REFUSED one does since CLI 2.1.250, see the `peer/messageStatus` row), so any other value would be the wire's own lie. Target resolution copies `thread/attach`'s rule exactly — a SIMULTANEOUS filter over `sessionId`/`pid`/`address`/`name`, where more than one match is `-32602` carrying the matches rather than a precedence, because a wrong guess delivers into somebody else's session. The envelope is assembled in the CLI's fixed attribute order (`from`, `from-session`, `hop-chain`, `from-name`, `from-mode`) and compared byte-exactly by the receiver, so attribute values are ESCAPED and a value carrying a C0 control character or DEL is refused `-32602` rather than sent — a silently downgraded envelope is a permission decision made on wrong information. **`from-mode` is always `prompting`**, and there is no parameter of any spelling that changes it: this gateway runs no model and asks no permission, and `fromThreadId` is attribution only (`from-session`, `from-name`), refused `-33004` for a thread this server does not hold and `-33006` for a fleet-origin one. The recorded consequence: every message this server sends is HELD by a `bypassPermissions` peer, and on a headless peer a hold expires. `msg_id` is a server-minted UUID — a non-UUID id comes back with no `orig_msg_id` and silently costs all correlation. `hop-chain` is never set (nothing here relays). Refuses `-32602` above a 60 000-character frame cap of our own, because the CLI's sender-side preflight belongs to the path we do not use and an oversize line meets the receiver's own cap, which drops it before the JSON is parsed and tells nobody; and `-33008` when no gateway is bound at all, since a send with no reply address is a message nothing could ever report on. `bridge:` targets — the cross-machine path, governed by a setting never measured here — are refused BY NAME rather than falling through to "no such peer" |
| `thread/crossSessionInbound/set` | appserver/settings.ts | `thread/crossSessionInbound/set` | inProcess | shipped(M8) — `{threadId, value}` → `{ok:true}`, plus the same `thread/settings/changed {source:"client"}` the other four settings setters emit, which now carries `crossSessionInbound` as a fourth knob on BOTH of its legs (`settings.ts`'s `broadcastSettings` and `router.ts`'s `routeSettingsMirror` — one payload shape, a full post-update snapshot rather than a diff, so a key on only one leg would be a client branch on which leg it happened to receive; read off the RECORD on both, since the engine's settings mirror never carries this key). **THE METHOD IS A TIGHTENING RATCHET, and that is its contract rather than a caveat.** Order the vocabulary by permissiveness — `accept` > `hold` > `refuse` — and a request equal to or STRICTER than the thread's current recorded value applies, while a LOOSENING request is refused `-32602` and is not attempted at all. Measured, never assumed (probes 120/120b: six single flips against a fixed baseline, each run carrying a control leg proving the send path worked in it, disposition read out of the receiver's own log): the CLI genuinely does re-read this key off the live flag layer `applyFlagSettings` writes, but only when the value gets stricter — `accept`→`refuse`, `accept`→`hold` and `hold`→`refuse` each changed the disposition of the very next inbound message, while `hold`→`accept`, `refuse`→`accept` and `refuse`→`hold` changed nothing at all. **The refusal is OURS, not the engine's**, and the distinction is the whole reason for it: the engine ACCEPTS a loosening write and then ignores it in silence, so forwarding one would mean replying `{ok:true}` for a change that did not happen — the one failure a security-shaped knob cannot have. Compared against the CURRENT recorded value rather than the LAUNCH one because every measured leg flipped exactly once from its launch value, leaving those two readings indistinguishable in the matrix; current is the reading that is safe under both, at the price of being stricter than necessary if the engine's real rule is against-launch. An EQUAL value applies idempotently and still announces — a tightening move of size zero, and refusing it would make a client's retry an error. A second move that partially loosens (`accept` → `refuse` → `hold`) is **UNMEASURED** — no leg ever probed a second flip — and the ratchet declines it before the engine could show us. Widening therefore needs an engine BUILT with the wider value, which today means admitting a thread at it (`thread/start`/`thread/resume`); the swap family already builds replacement engines from `record.config`, but no method exposes one for this key. The comparison runs at ARRIVAL time, ahead of `record.chain`, so a refused request never serializes behind a running turn and never reaches the engine. An accepted move writes the engine first (`applyFlagSettings`, so a rejected write cannot move the mirror), then `record.crossSessionInbound` and `record.config` through `applyPeerPolicy` in one breath — the config write is what makes the change survive the next engine swap, which rebuilds from `swapBaseConfig(record.config)` and would otherwise restore the launch policy in silence. A move TO `refuse` settles any adopted turn (`cancelled`: this server stopped following it — the engine survives this flip, unlike at the close and swap sites that pair the same two calls) and then uninstalls the arrival observer, which `peerInbound.ts` installs conditionally on the policy. There is deliberately NO install branch: loosening is refused, so no transition this ratchet admits can ever need one. `thread/settings/apply`'s reservation of the same key SURVIVES this method rather than being replaced by it — that write reaches the same flag layer with no ratchet, no mirror write and no announcement. A fleet thread answers `-33006` in the handler (this server did not build that engine, cannot inject a settings key into it, and the fleet engine deliberately lacks `applyFlagSettings` entirely); a chain-deferred throw from an engine that died after dispatch's arrival-time gate answers `-33005`, not `-32603` |
| `peer/messageStatus` | appserver/server.ts | `peer/messageStatus` | N/A | shipped(M8) — NOT a method: the delivery-status receipt, `{msgId, status, reason?, from, receivedAt}`, routed to the CONNECTION that sent the `peer/send` and dropped when that connection is gone (this is that request's delayed completion, not thread state, which is why it is connection-scoped where every other channel here is subscriber-scoped). The full status vocabulary is declared because the CLI's control frame declares it, but the measured truth is narrower and stated here: **`held`, `expired` and `denied` are observed** — a DELIVERED message produces no receipt at all, so silence remains the success path, and `held` is therefore the one status that does NOT release its correlation entry. **A REFUSED message was silent too until CLI 2.1.250** (the SDK bundles the engine: 0.3.237 ships 2.1.237, 0.3.250 ships 2.1.250). Since 2.1.250 a send into a `crossSessionInbound: "refuse"` receiver comes back to its sender as one `expired` receipt whose `reason` says the recipient is not accepting cross-session messages — measured 2026-08-29 by LEG 8 of `test/live/appserver-cross-session.test.ts`, which now asserts the receipt's status, its correlated `msgId` and a non-empty reason while deliberately leaving the reason's English unpinned. **The refusal itself did not loosen**: no `thread/peerMessage`, no turn, no items, and nothing in the receiver's own transcript, all still asserted by that leg. What changed is only that a sender can now LEARN of a refusal, where before a refusal and a successful delivery were indistinguishable from the sending side. `denied` was added by probe 120b (2026-08-28), which found that TIGHTENING a receiver's `crossSessionInbound` is retroactive: flipping to `refuse` disposed of a message already held and sent its sender a `denied` receipt, so a held entry can be resolved by a policy change on the far side rather than only by the deadline. Correlation is retained for an absolute 30 minutes (six times the CLI's default hold deadline, and FIXED rather than derived from this process's env, since the deadline belongs to the receiver), capped at 256 per connection and 4096 server-wide, and an entry evicted by either bound — or expired by shutdown's own sweep — notifies its sender `dropped` rather than vanishing |
| `thread/peerMessage` | appserver/peerInbound.ts | `thread/peerMessage` | inProcess | shipped(M8) — an inbound peer message ARRIVED. Fires on the replayed user frame, once per message, to the thread's SUBSCRIBERS (content, not existence — `watchThreads` is existence fan-out, a distinction the images round established). Recognition is the SDK's own `origin.kind === "peer"` and nothing else, decided by ONE function (`peerArrival`, src/peer/address.ts) that the cold replay path asks too, so the live and replayed item for one arrival cannot drift; Task 10d removed the envelope-text fallback that used to stand in for a missing `origin`, because measured over this machine's 64 transcript files only 12 of the 52 user rows carrying a complete envelope are real arrivals — the rest quote one — and the SDK's own rule is that an absent `origin` "is treated as unattributed and fails closed". Carries `origin` VERBATIM rather than re-derived, because `verifiedPeerPid` is the one field the kernel vouches for and the only non-forgeable identity in the exchange — `from` is sender-authored. **Params are `{threadId, arrivalUuid, origin, text}`**, and BL7 (#63) is what added the fourth: **`origin` is the engine's verbatim delivery provenance — in a collapsed batch its `body`/`msg_id` name the CAUSING message, not this arrival — while `text` is what THIS arrival says: the frame's own resolved text, identical under the same `arrivalUuid` on the announcement, the live item, the projected row and the replayed row.** The two are deliberately NOT reconciled: reconciling would invent an attribution the data does not contain (probe 121, verdict C — per-message identity inside a batch is non-bijective, and text coverage is the claim). Before that field existed a client had only `origin.body` to render, so a collapsed batch showed the causing message N times while the item channel showed each frame's own text — the defect #63 names, closed by ADDING a field rather than by editing the verbatim one. **No `turnId`, and there cannot be one**: at arrival the message's fate is undecided (fold, batch, or a turn whose id does not exist yet), so the field could only be fabricated, delayed, or null. `arrivalUuid` is the replayed frame's OWN uuid, which is also the id of the `userMessage` item the arrival produces, so a client deduplicates against `thread/read` with it |
...
`thread/crossSessionInbound/set`, was registered nowhere and rowed nowhere for as long as its measurement
...
What those two lines say at this sweep (**M8 inbound-policy setter, 2026-08-28** — restated per landing,
never trusted between them): **110 rows, 107 of them shipped** (15 `shipped(M1)`, 32 `shipped(M2a)`, 32
`shipped(M2b)`, 7 `shipped(M3)`, 2 `shipped(M4)`, 9 `shipped(M5)`, 2 `shipped(M7)`, 3 `shipped(F10)` and
5 `shipped(M8)`), **zero `probe-gated`**, and the same
`N/A` trio as the last five sweeps: `seedReadState` (internal plumbing, no protocol method by design), `readFile`
(probe-dead at 0.3.220, see its row) and `stageImage` (host-local transport by design). No `unparsed`
bucket, so **an `unparsed` count above zero still means a row's status really is malformed.** One bucket
moved and only one: `inProcess`, 16 to 17, for `thread/crossSessionInbound/set` — a thread-scoped method
that refuses the fleet origin outright, since this server did not build that engine.

**M9 (arrival history, 2026-08-30) moves neither number, and that is the claim rather than an omission:**
still **110 rows over 73 registered methods**, every bucket unchanged. The milestone registers no method
and announces on no new channel — it adds two optional RESPONSE members to two methods this table has
rowed since M1 and M5 (`origin` on `thread/read`'s user items, `arrivals` on the read and search replies),
which is what "an arrival rides its anchor row" buys: a feature that changes what history CONTAINS without
changing what the protocol EXPOSES. The gate was re-run at the landing and prints the same two lines.

The sweep before it (**M8 cross-session landing, 2026-08-27**) read **109 rows, 106 shipped**, with
...
of them. Its origin line moved in the `N/A` bucket, 15 to 18 — the peer domain's three method-and-receipt
rows name no thread, which is the same reading the peer section's prose gives for a different reason on
each of them — and then in `inProcess`, 15 to 16, for Task 10b's `thread/peerMessage`, which names a
thread and is announced only on threads this server itself admits (a fleet record seeds `DEFAULT_INBOUND`,
so it observes nothing).
...
The previous sweep read **105 rows, 102 shipped, zero `probe-gated`** at the M7 keyed follow-up
(2026-08-25), the run that emptied that bucket for the second time. Before that,
**102 rows, 98 shipped, 1 `probe-gated`** at the M7 dynamic-tools landing
