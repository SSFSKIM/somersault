...
M4's `review/start` is a **thirteenth** method of this kind — nothing walks it either, for the same reason
...
all rowed above — plus the 12 server-origin rows, which no walker produces, plus M4's 2 review rows, for
**93 rows** in all. (Recounted off the tables at M3 Task 12, which read "3 / 82" from the M2b close-out
...
agreed at 88 there, at 90 at M4 Task 9, at 91 at M5 Task 2 (`config/read`), and at 93 here (M5 Task 4's write pair). **The gate would not have caught a miscount at this landing**, which is
...
is why its mutual exclusion is a lockfile rather than anything the origin gate could express. The rows that remain
...
the live surface (N registered methods)"), it read 51 at the M2b close-out, 58 at M3's, 59 at M4's, 60 at M5 Task 2's `config/read` and 62 at M5 Task 4's write pair, and
the run is
