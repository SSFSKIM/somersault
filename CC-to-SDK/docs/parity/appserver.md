...
all rowed above — plus the 16 server-origin rows, which no walker produces, plus M4's 2 review rows, for
**97 rows** in all. (Recounted off the tables at M3 Task 12, which read "3 / 82" from the M2b close-out
...
agreed at 88 there, at 90 at M4 Task 9, at 91 at M5 Task 2 (`config/read`), at 93 at M5 Task 4 (the write pair), at 94 at M5 Task 7 (`thread/search`), at 95 at M5 Task 8 (`thread/searchOccurrences`) and at 97 here (M5 Task 9's archive pair). **The gate would not have caught a miscount at this landing**, which is
...
What those two lines say at this sweep (**M5 Task 9, 2026-08-19** — restated per landing, never trusted
between them): **97 rows, 95 of them shipped** (2 of those `shipped(M4)`, 7 `shipped(M5)`), the remaining two being the
...
a fleet session is as shelvable as an inProcess one because shelving never reaches an engine at all. The rows that remain
...
the live surface (N registered methods)"), it read 51 at the M2b close-out, 58 at M3's, 59 at M4's, 60 at M5 Task 2's `config/read`, 62 at M5 Task 4's write pair, 63 at M5 Task 7's `thread/search`, 64 at M5 Task 8's `thread/searchOccurrences` and 66 at M5 Task 9's archive pair, and
the run is
...
`thread/archived` and `thread/unarchived` — which have no rows of their own yet: they are emitted here and
documented on the two archive method rows above, and the filter that consumes them (`thread/list
{archived}`) is what earns them their own rows. Before those, **27**: 26 across all of M3 — `thread/closed`
