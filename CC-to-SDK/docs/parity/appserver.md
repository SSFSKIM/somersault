...
M4's `review/start` is a **fifteenth** method of this kind — nothing walks it either, for the same reason
...
all rowed above — plus the 14 server-origin rows, which no walker produces, plus M4's 2 review rows, for
**95 rows** in all. (Recounted off the tables at M3 Task 12, which read "3 / 82" from the M2b close-out
...
agreed at 88 there, at 90 at M4 Task 9, at 91 at M5 Task 2 (`config/read`), at 93 at M5 Task 4 (the write pair), at 94 at M5 Task 7 (`thread/search`) and at 95 here (M5 Task 8's `thread/searchOccurrences`). **The gate would not have caught a miscount at this landing**, which is
...
What those two lines say at this sweep (**M5 Task 8, 2026-08-19** — restated per landing, never trusted
between them): **95 rows, 93 of them shipped** (2 of those `shipped(M4)`, 5 `shipped(M5)`), the remaining two being the
...
has. Task 13's `thread/shellCommand` (`both`), Task 14's `thread/reopen` (`inProcess`) and M5 Task 8's
`thread/searchOccurrences` (`both`) are the only three server-origin rows with a real origin scope; every
...
process does. The rows that remain
...
the live surface (N registered methods)"), it read 51 at the M2b close-out, 58 at M3's, 59 at M4's, 60 at M5 Task 2's `config/read`, 62 at M5 Task 4's write pair, 63 at M5 Task 7's `thread/search` and 64 at M5 Task 8's `thread/searchOccurrences`, and
the run is
