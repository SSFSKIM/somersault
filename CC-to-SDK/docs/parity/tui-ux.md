...
**Three surfaces are deferred with reasons, and enter the tail list at the foot of this file** (spec § 5):
`CH30` tab status (`OSC 21337`), `CH34` the iTerm2 progress bar (which rides the same `OSC 9` code as the
notification, so the writer slot is reserved), and `TH3` tier 2 (the `OSC 11` background query, the only
surface in the wave needing a reply read back off the tty).
...
- **`CH34` iTerm2 progress bar.** It rides the *same* `OSC 9` code as the iTerm notification
  (`Onr = {NOTIFY:0, BADGE:2, PROGRESS:4}`, `Dnr = {CLEAR:0, SET:1, ERROR:2, INDETERMINATE:3}`, L188791),
  so the escape-builder unit already reserves the writer slot — this is a small follow-up rather than a new
  investigation. No row of its own.
