...
| 4. Modals / overlays | ~60% | ~88% | ~88% (4 new W3 rows — see W3 recount note) | ~50% | ~50% | ~50% (untouched by F4) | **~50%** (untouched by F5) | **~72%** (F6's dialogs wave: 6 rows rise, 2 new rows, 1 leaves the denominator — see the F6 recount) | **~73%** (Wave T adds two new rows — the consult footer at 🟡, the bypass consent gate at ✅) | ~71% (Wave C closes a fifth model-picker gap and adds one new 🟡 row for the effort dialog — the score falls because the denominator grows) | **~72%** (wave 2 rebuilt the resume preview and made Ctrl-C reach the exit over overlays; both enter as new rows, the preview 🟡 with its residues named and the Ctrl-C row ✅ after the delta's review found its one named residual — the `?` overlay — already closed by a binding) |
...
| **Overall (unweighted avg of the 7 categories)** | | | | | **≈ 75.1% → ~75%** — (87.5 + 67.2 + 65.0 + 71.7 + 84.1 + 66.7 + 83.3) ÷ 7 |
