...
| `CM33` — mouse hover/click on popup rows | **🚫 → ❌ at F9 (2026-08-22): the rationale is dead.** F9 T-MOUSE gave ccx real terminal mouse-mode ownership — the fullscreen transcript's `useMouseSink` registry, hover/click/drag, all live — but that ownership stops at the transcript region. The composer's suggestion popup (`DXe`) has no hit-map, no mouse sink, and no hover/click handling of its own; a click or hover over a popup row does nothing. Genuinely not built and now genuinely reachable — the transcript's own hitmap machinery is the template a popup implementation would extend | ❌ |
...
| K22 | The `Scroll` context (wheel, `ctrl+home`/`end`, shift-arrow selection, copy) | **❌ → 🟡 at F9 (2026-08-22):** the rationale is dead — ccx has terminal mouse-mode ownership now — and the selection-lifetime half of this context shipped: Ctrl+C copies the selection when `copyOnSelect` is off and clears it when on, any ordinary key clears it, and canon's allow-list (`escape`, `pageup`/`pagedown`, `ctrl+home`/`ctrl+end`, shift/meta arrows+home/end) does not. It ships as a pre-table hook (`useSelectionLifetime` in `KeymapProvider.tsx`), not a bindable `Scroll`-context action — invisible to `~/.claude/keybindings.json` and to the `?` shortcuts grid. **Still missing:** a named `selection:clear` action upstream itself binds, keyboard-driven shift-arrow selection EXTENSION (ours is mouse-drag-only), and wheel/`ctrl+home`/`ctrl+end` scoped to a dedicated `Scroll` context rather than riding the existing `Transcript` context (`K30`). See F9 T-MOUSE task 7 report | 🟡 |
...
| Mouse in fullscreen (`D7`–`D9`) | 🟡 | — | **New row, fullscreen wave — entering the denominator, and the largest single thing ccx does not have in this mode.** Upstream's fullscreen is fully mouse-live: wheel scroll (with `wheelScrollAccelerationEnabled` ramping), row click, hover, drag selection, auto-copy. Ccx has none — stock Ink 5.2.1 ships no mouse layer — and the wave deferred the whole class to M4 as the largest separable chunk. It was excluded before **only** because ccx had no alternate screen to own mouse modes in; the renderer re-arms mouse state on resize by contract now, so the class is plainly reachable. §1a's `K22` and `CM33`'s popup hover/click are members of this class, carried here rather than re-scored inside §1a's own ledger this pass. **❌ → 🟡 at the tool-stream wave (2026-08-19): the CLICK half shipped.** SGR reports decode into a typed mouse event in the raw-stdin parser (with the `& 64` and `& 128` guards that keep a five-button mouse's side buttons from decoding as a left click), route to an innermost-wins `useMouseSink` registry rather than into the binding table, resolve through a row map the viewport publishes for the frame it just painted (display width plus gutter columns, never character count), and toggle a cluster's expansion. **The rest of the v1 cut list SHIPPED at F9 T-MOUSE (2026-08-22); still 🟡, on one narrower residue.** Arming is now the full four-mode string (`?1000h ?1002h ?1003h ?1006h`) with both env opt-outs preserved byte-exact (`CLAUDE_CODE_DISABLE_MOUSE_CLICKS=1` → the old two-mode string; `CLAUDE_CODE_DISABLE_MOUSE=1` → nothing). Motion decodes to typed `motion`/`drag` events; hovering a dim row un-dims it and swaps in the hover-bg token, restoring on move-off (live-verified in a real pty: `\x1b[2m` stripped and restored, a `userMessageBackground` band swapping to its hover token by exact RGB triplet); a drag paints `selectionBg` across soft-wrapped rows and release copies the joined plain text (styling stripped, no newline at the wrap point); double-click selects the word under canon's char class, triple-click the line; a completed sweep pre-empts the fold-toggle/caret-move a plain click would otherwise fire; Ctrl+C copies or clears depending on `copyOnSelect` (default true), an ordinary key clears, canon's allow-list (`escape`, `pageup`/`pagedown`, `ctrl+home`/`ctrl+end`, shift/meta arrows+home/end) does not; auto-copy fires once per sweep on both clipboard channels at once (native `pbcopy`/`wl-copy`/`xclip`/`xsel`/PowerShell **and** OSC 52, including the `$TMUX` DCS-passthrough and `$STY` DCS-chunked forms) with canon's three-variant toast; click-to-caret resolves a composer click to the correct character across wrapped lines. Live-verified end to end over a real pty in isolated tmux: motion sweep un-dim/restore, press-drag-release paint + auto-copy + toast + the tmux-wrapped OSC 52 write captured raw off stdout, double/triple-click + Ctrl+C lifecycle, both env opt-outs byte-exact, click-to-caret landing between two known characters — all confirmed dead under `scroll`/`off`. **Recorded deliberate deltas, none of which is why this stays 🟡:** hover keys off the row-cluster (`itemKey`), not per-widget as canon's ~20 hover sites do — ccx has no layout tree for finer targeting (`CM33`'s popup-row hover/click is the still-unbuilt member of this class, tracked on its own §1a row); selection scope clamps to the visible region's columns, where canon's per-element walk needs a tree ccx doesn't have; a row that scrolls off mid-selection is not snapshotted, unlike canon's `Cka`; a fold-header or OSC-8 link-label segment paints the selection at whole-run width rather than char-precise (bounded to one row, verified harmless); the screen (`$STY`) DCS chunk size (76 base64 chars) is a documented convention, not a recovered canon constant. **The one residue that keeps this row at 🟡:** click-to-caret's row arithmetic assumes the composer is the dock's sole occupant above the footer — a live-turn spinner row, an open task panel, or a hoisted suggestion palette shift the composer's true screen row by their own height, uncounted, so a click during one of those states can land on the wrong character instead of failing safe (every other non-addressable case does fail safe). Canon's clickable error/truncated result rows are also still unbuilt — v1 makes only fold clusters clickable. Evidence: `docs/superpowers/specs/2026-08-22-f9-wave-design.md` §T-MOUSE, `.doperpowers/sdd/2026-08-22-f9-t-mouse/task-{1..8}-report.md` |
