...
> **2026-08-19 (post-tool-stream wave)**; each recount block below carries its own date, and older ones are
> left as written rather than back-edited. **Citation target moved with the tool-stream wave:** new
> fullscreen work is grounded in **2.1.234** (`cli.pretty.js`), while citations already shipped against
...
| Mouse in fullscreen (`D7`–`D9`) | 🟡 | — | **New row, fullscreen wave — entering the denominator, and the largest single thing ccx does not have in this mode.** Upstream's fullscreen is fully mouse-live: wheel scroll (with `wheelScrollAccelerationEnabled` ramping), row click, hover, drag selection, auto-copy. Ccx has none — stock Ink 5.2.1 ships no mouse layer — and the wave deferred the whole class to M4 as the largest separable chunk. It was excluded before **only** because ccx had no alternate screen to own mouse modes in; the renderer re-arms mouse state on resize by contract now, so the class is plainly reachable. §1a's `K22` and `CM33`'s popup hover/click are members of this class, carried here rather than re-scored inside §1a's own ledger this pass. **❌ → 🟡 at the tool-stream wave (2026-08-19): the CLICK half is built end to end.** SGR reports decode into a typed mouse event in the raw-stdin parser (with the `& 64` and `& 128` guards that keep a five-button mouse's side buttons from decoding as a left click), route to an innermost-wins `useMouseSink` registry rather than into the binding table, resolve through a row map the viewport publishes for the frame it just painted (display width plus gutter columns, never character count), and toggle a cluster's expansion — press and release must land on the same cell **and** still resolve to the same cluster, which is what survives streaming moving the document under a held button. Verified live: the premise run drove a real mouse press/release through a terminal emulator armed with exactly our two modes and through tmux into a pane app; the click cells themselves are replay-verified over a real pty. **The named missing arms are the v1 mouse cut, all deliberate and priced in the spec's §2:** arming stays `?1000h ?1006h`, so no motion tracking and therefore no hover brighten, no drag-selection engine, no auto-copy-on-select, no click-to-position-cursor in the composer, and no expanded-row background tint (a line-based row renderer has no per-item background span). Canon's clickable error/truncated result rows are also unbuilt — v1 makes only fold clusters clickable |
