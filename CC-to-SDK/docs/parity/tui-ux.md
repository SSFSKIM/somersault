...
> **2026-08-11 (post-Wave 2 delta)**; each recount block below carries its own date, and older ones are
> left as written rather than back-edited.
>
...
| **Overall** | **~46%**<br>*(impact-weighted)* | **~83%**<br>*(impact-weighted)* | **~88%**<br>*(plain row count)* | ~65%<br>*(plain row count)* | ~65%<br>*(plain row count)* | ~66%<br>*(plain row count)* | **~67%**<br>*(plain row count)* | **~71%**<br>*(plain row count)* | **~71%**<br>*(plain row count)* | **~75%**<br>*(plain row count)* | **~75%**<br>*(plain row count)* |
...
| Category | ✅ | 🟡 | ❌ | non-🚫 rows | Score |
|---|---|---|---|---|---|
| 2. Transcript | 13 | 13 | 3 | 29 | 19.5/29 = 67.2% |
| 3. Status / chrome | 10 | 6 | 4 | 20 | 13/20 = 65.0% |
| 4. Modals / overlays | 11 | 10 | 2 | 23 | 16/23 = 69.6% |
| 5. Slash commands | 16 | 5 | 1 | 22 | 18.5/22 = 84.1% |
| **Overall (unweighted avg of the 7 categories)** | | | | | **≈ 74.8% → ~75%** — (87.5 + 67.2 + 65.0 + 69.6 + 84.1 + 66.7 + 83.3) ÷ 7 |
...
| §5 | `/copy` | ✅ | 🟡 | **Repaired and marked down in the same pass, and the order matters: the repair is what named the gap.** The defect family was real and is dead — `/copy` reads the live wire rather than a stale index (s2qa5-21), resets at `replaceDocument` so a cleared conversation's text can no longer reach the clipboard (s2qa5-22, a direct inversion of Wave S's measurement-dies-with-its-conversation rule), filters API-error frames on **both** paths, and prints canon's `No assistant message to copy` where ccx said `nothing to copy`. Reading canon for that empty string is also what surfaced **`/copy N`** (L444892/445068): upstream's command takes an index over a 20-deep list of recent assistant messages, and ccx only ever copies the newest. That is reachable and unbuilt, so the row cannot hold a ✅. One recorded internal asymmetry, not an upstream gap: the live path filters on the error flag and the disk path on the CLI's own `<synthetic>` marker, because the session store strips the flag from persisted rows |
| §4 | `/resume` preview body | — | 🟡 | **New row, no prior row existed** — the picker row below scored `Space` preview as a 12-line tail, and the tail was raw persisted text that bypassed the species router. The preview now IS the transcript: `projectCompact(replayDocument(msgs, {id, width}))` composed with `projectPending`, which is what stops a session ending in a tool call from previewing without its trailing fold run (compact alone withholds it). No `<command-name>`/`<local-command-stdout>` envelope leaks (s2qa4-13, closed), tail-anchored under the pane budget with `↑ N more above` in the package's one overflow spelling, and the count floored to `N+` when the 200-message window cut the input. The count-vs-rows invariant survives on one predicate. **Missing arms:** canon **replaces the picker with a full-screen rendered transcript** under its own footer (L476605) where ccx renders in-pane — a recorded divergence (D-W9) but a backlog item, not a deliberate end state, so it is scored as a gap; and an **image-only session renders the empty state over a nonzero count**, because the shared predicate counts an image turn upstream also counts while the pane has no text to draw for it |
| §4 | Ctrl-C exit while an overlay owns input | — | 🟡 | **New row, no prior row existed.** Canon binds the 800 ms exit latch on a dialog's own scope and a second press exits (L184112); ccx's overlay contexts declared `"ctrl+c": null`, and an unbound key is a *consumed* key, so the exit arm was unreachable from six surfaces and the hint was gated out even when armed (s2qa4-11 — claude exits 0, ccx did nothing). The nulls are gone from all six (Select, Settings, Help, MessageSelector, EffortDialog, SessionPicker — Transcript and HistorySearch *rebind* and were already fine), an armed exit now prints its hint over pane-owning surfaces, and the first press clears the composer draft **only when the composer owns input** (D-W10, from canon's own `Pee`/`h5u` split: over a dialog the first press arms and does nothing else — the old behaviour silently destroyed a parked draft the user could not see). **Missing arm:** the `?` shortcuts overlay still swallows `ctrl+c`, because it takes a preemptive swallow scope above the binding table entirely (`KeymapProvider`'s `swallowContexts`) — genuinely out of a table-level fix's reach, backlog. That residual is scored **here and only here**; §1's `?`-overlay row is re-noted rather than marked down, so the same gap is not counted twice |
| §2 | Meta user frames (`isMeta` / `<system-reminder>`) | — | ❌ | **New row, no prior row existed** — found by Task 8's review while pinning the preview's count predicate. Upstream excludes a meta user turn from the transcript entirely; ccx draws its raw text, in the resume preview **and** in the live replay path, so a `<system-reminder>` block appears as if a human had typed it. Nothing is built here: the species router has no meta arm (fix site `species.ts`/`rows.ts`). Whether an inherited disk row even keeps the flag hinges on `getSessionMessages` preserving `isMeta`, which is unprobed — that is a question about the shape of the fix, not about whether the gap is real, and the live path needs no probe to see it |
| §2 | Bare plan-rejection row copy | — | 🟡 | **New row, no prior row existed**, and it is the residue of the wave's own A4 fix rather than a standing gap: making a bare plan rejection END the turn (below) left the transcript printing ccx's generic interrupt substitution, `⎿ Interrupted · What should Claude do instead?`, where upstream prints `User rejected Claude's plan:`. The row renders and the turn ends correctly — this is the copy, not the mechanism — so 🟡, not ❌. Distinct from the plan dialog's row-3 hint, which the fixer **declined** to restore with correct reasoning (canon's longer copy advertises a feedback channel `canUseTool` silently drops); that one is a documented deliberate divergence and does not score |
...
- **§3 `statusLine`** — the payload now speaks canon's contract: `transcript_path` and `prompt_id` latched
  off the headless-firing `UserPromptSubmit` hook (probe `104-userpromptsubmit-transcript-path`), a
...
| `?` shortcuts / help menu | ✅ | — | **C5** `ShortcutsOverlay.tsx` — a real bordered overlay listing the keymap, opened by `?` on a genuinely empty composer; the U7 footer hint line stays alongside it. **fixed 2026-07-31 (F0, t5, KB6):** this row was scored ✅ but the overlay closed on **any** key, and that same key also fired `ChatApp`'s global chords underneath it (e.g. `Ctrl-O` would both close the overlay and open the transcript pager in one keystroke) — the correction had this at 🟡. It now closes on Escape only and swallows every other key, matching upstream's `Help` context (which binds only `escape`); a sabotage-verified honesty-audit test pins this. **Wave 2 (t3) — re-noted, deliberately not marked down:** the swallow is faithful for every key except one. Canon binds the double-press exit latch on a dialog's own scope, so `ctrl+c` `ctrl+c` exits from the help overlay there and does nothing here — but this overlay takes a *preemptive swallow scope* above the binding table (`KeymapProvider`'s `swallowContexts`), so dropping the context's `ctrl+c` null could not reach it. The gap is real, backlogged, and scored **once**, on §4's "Ctrl-C exit while an overlay owns input" row; counting it here as well would charge the file twice for one defect |
...
| Meta user frames (`isMeta` / `<system-reminder>`) suppressed | ❌ | — | **new row, wave 2 (t8 review), no prior row existed.** Upstream drops a meta `user` turn from the transcript entirely — it is scaffolding the model reads, not something a human typed. ccx renders its raw text, in **both** the `/resume` preview pane and the live replay path, so a `<system-reminder>` block appears in the conversation as a prompt. Nothing is built: `species.ts` has no meta arm and `sessions/rows.ts` inherits the omission (the two fix sites). Distinct from the sentinel-router row above, which is about `ERe`'s content-shaped exits — this is the frame-level flag that precedes them. One open question about the *fix*, not the gap: whether `getSessionMessages` preserves `isMeta` on a replayed disk row is unprobed, so a disk-side suppression may need a content fallback; the live path needs no probe to show the defect |
| Bare plan-rejection row copy | 🟡 | — | **new row, wave 2 (acceptance fix), no prior row existed.** Rejecting a plan with no feedback now ends the turn (§8's plan row), and what the transcript prints for it is ccx's generic interrupt substitution — `⎿ Interrupted · What should Claude do instead?` — where upstream prints `User rejected Claude's plan:`. 🟡 rather than ❌ because the row renders, the turn ends and the mechanism is right; only the copy is ours. Not to be confused with the plan dialog's row-3 hint, which the wave's fixer **declined** to restore on correct reasoning — canon's longer copy advertises a feedback channel `canUseTool` silently drops here — and which therefore stands as a documented deliberate divergence, unscored |
...
| Random thinking verbs | 🟡 | **U2**. **F0 correction:** mirrors §3's "Spinner thinking verbs" finding — 186 upstream verbs not 187 (we carry one extra), and the random verb is upstream's last fallback, not its primary source (the active todo's `activeForm` goes first). **Wave C (t6): the count half is closed** — the invented 187th verb (`"Evaporating"`) is deleted and the list is pinned at 186. Still 🟡 for the source half, which is the same half §3's row keeps |
