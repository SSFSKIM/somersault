...
| §2 | Meta user frames (`isMeta` / `<system-reminder>`) | — | ❌ | **New row, no prior row existed** — found by Task 8's review while pinning the preview's count predicate. **Probe 107 (2026-08-12) overturned the disk half of this row's claim:** `getSessionMessages` DROPS meta rows entirely (53/53 and 14/14 meta uuids absent across two real transcripts; zero `<system-reminder>` hits in reader output with `includeSystemMessages` both ways; every returned row projects onto one fixed 7-field shape) — so the resume preview and disk replay were never reachable by an `isMeta` row, the reviewer's reproduction was a synthetic fixture, and `sessionPickerModel.ts:161`'s `if (r.isMeta)` predicate is dead code (cleanup pending). The row stays open for the LIVE vector only: reminder text riding inside live-turn stream rows — unverified, and the sole remaining fix site if real. SDK-minted sessions record injected reminders as `type:"attachment"` rows, which the reader also drops |
...
| Meta user frames (`isMeta` / `<system-reminder>`) suppressed | ❌ | — | **new row, wave 2 (t8 review), no prior row existed.** Upstream drops a meta `user` turn from the transcript entirely — it is scaffolding the model reads, not something a human typed. **Probe 107 (2026-08-12) settled the open question and overturned the disk claim:** `getSessionMessages` returns NO meta rows at all (row-dropped, not flag-stripped — two real transcripts, 53+14 meta uuids, zero returned; zero `<system-reminder>` text in any returned row), so the `/resume` preview and disk replay are already clean by construction and the t8 reviewer's reproduction was synthetic. What remains of this row is the LIVE vector only — reminder text arriving inside live-turn stream rows — plus dead-code cleanup (`sessionPickerModel.ts:161`'s never-true `isMeta` predicate). Distinct from the sentinel-router row above, which is about `ERe`'s content-shaped exits |
