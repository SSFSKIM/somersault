...
| permission-ask `default_to_no` (0.3.251) | 🔬 | Ask payload flag: the prompt must not be approvable by a stray keystroke (opens on decline, no digit shortcut). Passthrough-shaped for the daemon permission broker and ccx dialogs — relay it and let hosts honor it; near-zero build once probed |
