...
| Cross-session messaging: receive side (`SDKMessageOrigin` peer, `crossSessionInbound`, `queued_notifications`) | 🚫 | probe 110: headless SDK sessions are NOT addressable — absent from every peer's roster, capabilities = [interrupt_receipt_v1, interrupt_cancel_queued_v1, msg_lifecycle_v1] with NO queued_notifications, and a marker sent at one never arrived. The receive fabric (registry entry + messaging socket) belongs to Claude Code hosts; making ccx sessions addressable needs its own grounding pass (registry/socket protocol in Claude Code Src) |
