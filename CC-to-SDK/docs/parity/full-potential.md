...
Grouped by the docs' own capability themes. 126 rows. "Evidence" cites probes
...
| result `queued_turn_count` + `user_message_uuid` echo on error results (0.3.251) | 🟡 | **Probe 123 (2026-08-30): PRESENT headless — `queued_turn_count: 0` on a success result, key present not absent — but the >0 reading went unexercised for an instructive reason:** the two extra uuid-stamped sends pushed mid-turn FOLDED into the running turn (the 118-mapped fold outcome) instead of queueing, so the queue was honestly empty at result time and no second result ever came. The field's semantics held exactly (0 = nothing pending); forcing >0 needs an interleaving the fold matrix says queues rather than folds. **`TurnOutcome.queuedTurnCount` SHIPPED Wave C** (additive on the lib Session's settle path, key absent when the frame carries none); the error-result `user_message_uuid` echo remains unexercised (needs a deliberate failure turn). The 0.3.250 filing's semantics and open question stand: it counts pending user SENDS, not remaining results (queued sends coalesce, system-generated entries are skipped, declared absent on fatal startup results), and whether the CLI's count agrees with our own queue depths mid-burst (`useChat`'s `queue`/`submitCount`, the app server's queued sends) is still what a >0 reading must settle — two disagreeing depths on one status bar is worse than one |
| `user_message_uuid` turn-binding stamp (0.3.250, three carriers) | 🔬 | **Found by the 0.3.250 full-text `sdk.d.ts` diff, NOT by the four-surface scan**. Echoes back the `submitMessage` `options.uuid` of the send a turn answers, so a consumer can bind reply to send WITHOUT waiting for the result. Three carriers, one stamp each: the turn's first assistant message in complete-message mode, the first non-ping stream event under `--include-partial-messages` (which then takes the stamp instead), and the error-result variant (carried alone there, since an error turn has no `request_sent_wall_ms`). Wrapper-level sibling, never inside `message.content`, so it is not replayed to the model. Declared absent on every later frame, on subagent frames (`parent_tool_use_id` set), on synthetic/scheduled turns, on turns with no client uuid, and from older producers. **Unprobed, zero consumers.** The premise this would settle is real — the app server today attributes a streaming reply to a send by ordering and waiter identity (`session.ts`'s submit waiter, `onUnclaimedResult` for peer-initiated turns), and an explicit join key is exactly what a peer-initiated or adopted turn lacks — but which carrier arrives in OUR partial-message configuration is a measurement, not a reading |
...
| SDK-MCP per-server `timeout` (0.3.251: `createSdkMcpServer` option + `McpSdkServerConfig.timeout` + initialize `sdkMcpServerConfigs`; also on `McpStdioServerConfig`) | ✅ | **Probe 124 (2026-08-30): ENFORCED, to the decimal.** A server declared `timeout: 5000` had its 60s tool killed at 5.0s with `is_error:true` and the exact text `MCP server "slowsrv" tool "slow_wait" timed out after 5s`; a sibling declared `timeout: 500` was IGNORED as documented (below the 1000ms floor — its 3s tool completed at 3.0s). **SHIPPED Wave C** as `HarnessConfig.mcpToolTimeoutMs`: a default stamped onto every config-carried server lacking its own `timeout` (mcpServers/taskTools/swarm/dynamic overlay; a declared value always wins; injected cc-context/cc-compact exempt — sub-ms calls). **Review fix-wave:** the stamp now covers ALL FIVE server shapes — the review caught that `timeout` was declared on http/sse configs BEFORE 0.3.237, invisible to a delta-derived reading of the schema. Incidental census datum: on a bare 0.3.251 session the SDK-MCP tools arrived DEFERRED — the model had to `ToolSearch` (`tool_reference` results) before calling them, so a harness that needs a custom tool called reliably on turn 1 should account for the deferral round-trip. Still unpinned from the 0.3.250 filing: the timeout binds at FIRST REGISTRATION (changing it on an already-registered server no-ops until remove/re-add) and progress notifications do not extend it — doc claims, not yet measurements |
...
| permission-ask `default_to_no` (0.3.251) | 🚫 | **Out of reach on our channel (checked against 0.3.251, Wave C):** the field lives only on the raw `SDKControlPermissionRequest` wire frame; the SDK's `CanUseTool` options type (the callback our permission broker/gate is built on) does not forward it — signal/suggestions/blockedPath/decisionReason/title/displayName/description/toolUseID/agentID/requestId/matchedAskRule and nothing else. Nothing to relay until the SDK forwards it; re-check on future bumps. The filing's rendering obligation waits with it: our TUI permission dialog does default its selection and does take single-key answers, so the day the SDK forwards the field the dialog owes it a measured behaviour change |
...
| `perTaskStopAffordance` + `stop_task` control (0.3.251) | 🟢 | **Probes 126/126b (2026-08-30): `stopTask()` WORKS; the Options declaration is a headless no-op because the spare behavior is already on.** With attribution windows (interrupt → 10s observe → stopTask → 6s observe): a running background `sleep 90` was HELD through an interrupt in BOTH the declared and undeclared phases — the documented fail-closed kill never fired, most plausibly because the SDK transport declares the affordance itself (it hands every consumer `Query.stopTask`). `stopTask(id)` then emptied `background_tasks_changed` inside its own window, cleanly attributed. **Daemon `stop_task` op SHIPPED Wave C** (`{op:"stop_task", id, taskId}` → `supervisor.stopTask` → the lib `Session.stopTask` that already existed); interrupt-spares-tasks is the baseline semantics to encode in Session docs. **Review fix-wave (same day):** the daemon now DECLARES `perTaskStopAffordance: true` on every session it spawns (explicit factory value wins) — truthful, since its stop_task op IS the affordance — so the spare-on-interrupt behavior rests on the documented contract rather than the transport's incidental self-declaration; a first-class `HarnessConfig.perTaskStopAffordance` passthrough ships with it. Rough edge worth keeping: an interrupt landing mid-tool-call ended the turn as `error_during_execution` and the stream teardown was ungraceful in both phases (process exit 1 in one, an `ede_diagnostic` error result in the other) — the Session read loop should treat post-interrupt stream death as expected-noisy, not a crash. Filing caveats that survive the probe: first-attached-client wins on multi-client sessions, and a closed-input one-shot `-p` run kills hold-back tasks regardless |
| background-task `ambient`/`is_backgrounded`/`spawn_depth` + reinit snapshot (0.3.251) | 🟡 | **Probes 126/126b/127 (2026-08-30): the `task_started` fields are LIVE.** A background `sleep 90` carried `is_backgrounded:true` on its `local_bash` frame (`spawn_depth` correctly absent — it is local_agent-only); an Agent-tool spawn carried `task_type:local_agent, is_backgrounded:true, spawn_depth:1` — note **true**: the 2.1.232 background-by-default agent spawn holds on the SDK transport too, so a headless host must not assume a spawning tool call blocks on its subagent. `ambient` never appeared (no housekeeping task arose in the runs) and the repeated-initialize snapshot is still unprobed — it needs a reconnect rig, which is the app-server workstream's territory (their M-series has the multi-client initialize machinery); flagged to them rather than duplicated here. Per the filing, `ambient` marks housekeeping the CLI does not surface as user work (`skip_transcript` tasks, auto-started watchers) — the activity indicator's own exclusion list; the panel mapping for the two live fields also stands: `is_backgrounded` separates Ctrl+B'd foreground work from work born in the background, `spawn_depth` is the nesting the Background panel currently flattens |
| `background_tasks_changed` snapshot behind a repeated `initialize` (0.3.250) | 🔬 | **Found by the 0.3.250 full-text `sdk.d.ts` diff, NOT by the four-surface scan** — a delivery guarantee stated only in the message type's own doc, with no name attached. A host that re-initializes an already-running process (a repeated `initialize` control request, e.g. after reconnecting) is now sent a snapshot of the current set immediately behind that request's success response, EVEN WHEN EMPTY, so it need not wait for the next membership change; CLIs that predate this send nothing there. **Unprobed, unconsumed as a guarantee.** It answers a gap the level signal has carried since Wave 1 — the set is per-process and nothing is emitted at startup, so a reattaching client shows an empty Background panel until something changes — and it composes with the reinitialize row in §H: the same call that now re-registers hooks also repopulates this. A probe must confirm ordering against the success response and that the empty case really arrives, since "absent" and "empty" are the two states a reattached panel must tell apart |
...
| `ModelUsage.costBasis` + pricing/model settings keys (0.3.251: `modelPricing`, `modelPicker`, `modelSettings`, `promptCacheTtl`, `subagentPromptCacheTtl`) | 🟢 | `costBasis` ('list'/'managed'/'unknown') flows for free: `usage()` and every result's `modelUsage` are unknown-typed verbatim re-exports, so the field is already in consumers' hands (Wave C reasoning — no transform sits between the frame and the reader; nothing to build, nothing to pin). **`promptCacheTtl`/`subagentPromptCacheTtl` SHIPPED Wave C** as first-class `HarnessConfig` knobs mapping into `options.settings` (settings.ts, knobs.test.ts rows) — the actionable pair for a headless service on an API key. `modelPricing`/`modelPicker`/`modelSettings` stay settings-passthrough (advisory-adopted 158/158; picker/pricing are host-UI and managed-org surfaces we don't interpret). The 0.3.250 filing's semantics carry over: `'unknown'` is the load-bearing value (no pricing row and no built-in price matched, so `costUSD` is a GUESS at the default model's rate); overwritten per request like `canonicalModel`, absent until this process has priced a request for the model (e.g. right after `--resume`) and on older builds, where it reads as `'list'` — on the cost surfaces we ship (`usage()`, the OTel cost metrics, the turn meter) it is the difference between reporting a number and reporting a number we know to be a guess |
...
| initialize-response `hooks_applied` + `reinitialize()` hook re-registration (0.3.250) | 🔬 | **Found by the 0.3.250 full-text `sdk.d.ts` diff, NOT by the four-surface scan** — a field on the initialize response plus the SEMANTIC change it reports, and the semantic half is the one that already cost us a false comment. Over stdio the CLI now re-registers this query's hooks from a re-sent `initialize` (`hooks_applied`: true on the first initialize and on a repeat from the process owning the CLI's stdin, whose set REPLACES the earlier one; false when a repeat's hooks were ignored, as for a client joining a remote session another client configured; absent when the request carried no hooks, and on CLIs that predate the field — those ignored `hooks` on every repeat). Because it cannot tell whether the re-sent callback ids still name the same hooks, it also RESOLVES any hook callback it was waiting on: a pending `PreToolUse` is denied with a retry notice, a pending prompt is blocked. **Unprobed, zero consumers** — `session.ts`'s `reinitialize()` jsdoc was corrected to stop promising probe 38's mid-session safety for the hook half (the permission half still holds; the parked request is still deduped), but nothing reads the flag. This is the first thing in the bump that makes an EXISTING call less safe rather than offering a new one, so the probe owes two answers: whether our reinitialize path can race an unanswered hook at all, and whether replacing a live hook set mid-session is a lever worth having or only a hazard to route around. (2026-08-30 Wave B triage: deferred, together with the reinit-snapshot probe, to the app-server workstream's reconnect rig — no local rig can re-send `initialize` on a live stdio session) |
| `PreModelSwitch` / `PostModelSwitch` hook events (0.3.251) | ✅ | **Probe 121 (2026-08-30): fully ALIVE headless, all three answer paths.** A streaming session on haiku registered both as SDK-callback hooks and called `q.setModel("sonnet")` between turns: Pre AND Post fired with the complete payload (`from_model`/`to_model` resolved ids, `requested_model:"sonnet"`, `source:"sdk"`, `context_tokens:18643`, `prompt_cache_warm:true`, `cache_ttl:"1h"` on the subscription, `estimated_cache_write_usd:0.0746`, `pricing:"catalog"`) and the switch really landed — the next assistant frame ran on `claude-sonnet-5`. Pre answering `permissionDecision:"deny"` cancels the switch AND rejects the `setModel()` promise with `Model switch blocked by a PreModelSwitch hook: <reason>`; Post never fires; the session stays on the old model. `"ask"` behaves as its doc promises on a headless session: refused, same rejection shape. **SHIPPED Wave D** on exactly this contract: `HarnessConfig.modelSwitchPolicy` (`src/hooks/modelSwitch.ts` `buildModelSwitchHooks`) — allowModels family/exact matching, a warm-cache forfeiture cap (`maxCacheWriteUsd`), a guardTool-vocabulary `decide()` escape hatch under deny-wins composition, `annotate()` → Post additionalContext, and a throw-swallowed `onSwitch` tap; merged after user hooks in resolveOptions so it reaches harness/Session/daemon uniformly. Spec `2026-08-30-model-switch-governance-design.md`; unit 10 rows + live 2/2 (deny with prefixed reason + landed switch with both tap phases). **Review loop (3 codex rounds, 12 findings, all verified real, all fixed; converged when substance narrowed to boundary polish on the newest deltas):** async taps awaited; policy data-field validation at BOTH front doors (harness + daemon, shared schema); daemon-wide `DaemonOptions.modelSwitchPolicy` reaching cold spawns, warm-pool leases, restarts and rehydration alike; a sessionOptions factory returning `hooks` re-merges instead of clobbering governance (the SERVER_OWNED lesson at the daemon seam); blank allowlist entries can never match a null `requested_model`; annotations hard-capped at 2000 chars (mirroring classifierContext's own cap). Live 2/2 re-verified on the post-loop code |
| `SessionStart` resume-staleness fields (0.3.251: `seconds_since_last_response`, `context_tokens`, `prompt_cache_likely_expired`, `estimated_cache_write_usd`) | 🚫 | **Probe 122 (2026-08-30): unreachable on the channel we own.** An SDK-callback `SessionStart` hook fired on NEITHER a fresh start NOR a `resume` — consistent with the Wave 2 census (SessionStart fires headlessly only at the /compact boundary), which the premise row above forgot; 122 extends the dead verdict to the resume path where these fields would live. The fields may exist for interactive/settings-hook consumers, but the daemon registers hooks through Options callbacks, so resume-cost observability stays a non-surface for us. (Registration shape is not the suspect: the identical pattern fired PreModelSwitch in probe 121 the same day) |
...
**74% realized post-0.3.250** — 126 rows, 6 on the 🚫 floor, denominator 120. **The number went
DOWN and nothing regressed.** Two separate movements are behind it, and they are worth keeping apart.
First, the ~83% below was computed at Wave 4 and never recounted while M1–M8 shipped; recounting the
same file by the same rule the moment before this bump's rows landed gives **80.4%** (117 rows,
denominator 111), so ~3 points of the gap is ordinary denominator growth nobody had re-measured.
Second, this bump's own nine 🔬 rows cost **6 more points** on their own, because the numerator does
not move when the map grows: an SDK that ships faster than we probe lowers this score by existing.
That is the honest reading of the ritual's own premise — declared ≠ reachable, so a new declaration is
debt until a probe settles it — and it is why the percentage is a poor headline on its own. The
useful figure beside it is the 🔬 count: **10 unprobed rows**, 9 of them from one bump.

Prior recount: **~83% post-Wave-4** (was ~78% post-Wave-3: Wave 4 closed the Options long tail — all 63
...
score. The unrealized quarter no longer has the shape it had at Wave 4, when it was one long tail of
trivial knobs:

1. **Unprobed SDK additions** (the 🔬 rows) — now the largest single block, and the one that grows on
   its own schedule rather than ours. Nine arrived in the 0.3.237→0.3.250 bump alone; each is cheap to
   settle and none is buildable before it is. This is the block a wider drift scan would surface
   earlier (see the recommendation in the ritual's run log).
...
3. The Wave-4 knob tail is CLOSED — all declared Options fields are modeled, `perTaskStopAffordance`
   included, and the W4.1 pattern absorbs each new one in three lines.
...
**Re-checked at 0.3.250 — the floor is unchanged.** The full-text diff of 0.3.237→0.3.250 contains
exactly two `-` lines and both are widenings (`ManagedNotification.subkind` gains `'projects-relay'`;
`Settings.spinnerTipsOverride.tips` becomes optional and admits objects), so no exported name, `Query`
method or `SDKMessage` member was deleted and nothing left the floor by disappearing. Nothing joined
it either: none of this bump's nine additions was probed dead, because none was probed at all — a
🔬 row is not a floor row, and the two must not be allowed to blur.
