...
| `hooks_applied` on repeated initialize (0.3.251) | 🔬 | Over stdio the CLI now re-registers the re-sent request's hooks on reconnect (`hooks_applied: true` from the stdin-owning process; false = ignored, absent = older CLI) and self-resolves hook callbacks it was still awaiting (denies a pending PreToolUse with a retry notice). Matters to the daemon restart/resume path and to any future multi-client host; deferred with the reinit-snapshot probe to the app-server workstream's reconnect rig (2026-08-30 Wave B triage — no local rig can re-send initialize on a live stdio session) |
| permission-ask `default_to_no` (0.3.251) | 🚫 | **Out of reach on our channel (checked against 0.3.251, Wave C):** the field lives only on the raw `SDKControlPermissionRequest` wire frame; the SDK's `CanUseTool` options type (the callback our permission broker/gate is built on) does not forward it — signal/suggestions/blockedPath/decisionReason/title/displayName/description/toolUseID/agentID/requestId/matchedAskRule and nothing else. Nothing to relay until the SDK forwards it; re-check on future bumps |
