...
| result `queued_turn_count` (0.3.250) | 🔬 | **Found by the 0.3.250 full-text `sdk.d.ts` diff, NOT by the four-surface scan** — a new field on BOTH result shapes, so no exported name moved. Counts the user-initiated sends still waiting in the command queue when the result was produced: `>0` means at least one more user turn (and result) follows without further input, barring cancellation; `0` means none pending OR the session is ending (`end_session`, or a shutdown latched mid-turn, discards the backlog). It counts pending SENDS, not remaining results — queued sends coalesce into fewer turns — and skips system-generated entries; declared absent on fatal startup results and on surfaces with no command queue. **Unprobed, zero consumers** (`grep -rn queued_turn_count harness/src` → nothing). Worth settling rather than adopting, because we already keep a queue depth of our own on both sides (`useChat`'s `queue`/`submitCount`, the app server's queued sends): a probe must establish whether the CLI's count agrees with ours mid-burst, since two disagreeing depths on one status bar is worse than one |
...
| Per-server MCP tool-call `timeout` (0.3.250; two spellings) | 🔬 | **Found by the 0.3.250 full-text `sdk.d.ts` diff, NOT by the four-surface scan** — both spellings live on non-exported config types. `McpServerConfig.timeout` (ms) overrides `MCP_TOOL_TIMEOUT` for ONE server, and `initialize.sdkMcpServerConfigs[<name>].timeout` does the same for the in-process servers named in `sdkMcpServers` — sent as a separate field so an older CLI ignores it, with unknown names and mis-shaped values ignored rather than rejected. Hard wall-clock per call: progress notifications do NOT extend it, values under 1000ms fall through to the env var, and it binds at FIRST REGISTRATION — changing it on an already-registered server does nothing until the server is removed and re-added. **Unprobed, zero consumers.** This is the per-server answer to the row above (the process-wide `MCP_TIMEOUT` env knob), and it matters most for our five in-process servers, where a slow `cc-compact` and a fast `cc-context` currently share one deadline; the re-registration rule is what a probe must pin, since a config surface that silently no-ops on update is a worse knob than none |
...
| permission-ask `default_to_no` (0.3.250) | 🔬 | **Found by the 0.3.250 full-text `sdk.d.ts` diff, NOT by the four-surface scan** — a field on the permission-ask payload, alongside the existing `suppress_always_allow_rule`. True when the ask must not be approvable by a single stray keystroke (`PermissionAskDecision.defaultToNo`): a terminal-style prompt opens on its DECLINE option and takes no digit shortcut, and hosts are told not to pre-select approve. **Unprobed, zero consumers** — and it is the one row here with a rendering obligation attached, since our TUI permission dialog does default its selection and does take single-key answers. Probe must settle which asks actually carry it headlessly before the dialog changes behaviour; adopting it on the declaration alone would change which keystroke approves a tool call, which is precisely the class of change that needs a measurement first |
...
| `perTaskStopAffordance` (0.3.250; the one Options field this bump added) | 🔬 | **Found by the 0.3.250 full-text `sdk.d.ts` diff, NOT by the four-surface scan** for the CONSEQUENCE, though the Options field itself is the single name the scan did catch. Declares that this consumer renders a per-task stop control wired to the `stop_task` control request; when declared, an interrupt on an open-input session aborts only the turn and SPARES running background agents/workflows, to be stopped one at a time. **ABSENCE FAILS CLOSED** — the interrupt kills them, which is today's behaviour — and a closed-input one-shot run (`-p`) kills hold-back tasks regardless. First-attached-client wins; later initializes do not change it. **DECLARED AND WIRED, DELIBERATELY UNSET**: the W4.1 knob exists (`HarnessConfig.perTaskStopAffordance` → one `resolveOptions` line → `knobs.test.ts` row) and nothing in the repo sets it, so no behaviour has changed. Two reasons it stays off until probed: the claim is about OUR UI and is only true for the bundled REPL (`BgTasksPanel`'s `onStop` → `stopBgTask` → `session.stopTask` → `q.stopTask`), never for a library caller that ships no such control; and 🔬 here means what it always means — nobody has watched an interrupt spare a live background agent, only read that it would |
| background-task `ambient` / `is_backgrounded` / `spawn_depth` (0.3.250) | 🔬 | **Found by the 0.3.250 full-text `sdk.d.ts` diff, NOT by the four-surface scan** — three fields on the background-task frames. `ambient` marks housekeeping the CLI does not surface as user work (every `skip_transcript` task, plus auto-started live-update watchers) and rides BOTH the `background_tasks_changed` entries and `task_started`, which now also carries `is_backgrounded` (registered in the background vs. a spawning tool call blocking on it; a resumed subagent is always true, and a later move arrives as `task_updated` `patch.is_backgrounded`) and `spawn_depth` (1 for a top-level `local_agent` spawn, N+1 inside a depth-N agent; unset on other task types). **Unprobed and unharvested** — `bgTaskMeta.ts`'s comment was corrected to stop claiming the snapshot carries only `{task_id, task_type, description}`, but no field was wired. Each maps to a real panel question we answer worse today: `ambient` is the activity indicator's own exclusion list (we count watchers as user work), `is_backgrounded` separates Ctrl+B'd foreground work from work born in the background, and `spawn_depth` is the nesting the Background panel currently flattens |
...
| `ModelUsage.costBasis` (0.3.250) | 🔬 | **Found by the 0.3.250 full-text `sdk.d.ts` diff, NOT by the four-surface scan** — a field inside the per-model usage record `modelUsage` already carries. Says which price table the model's most recent request was priced at: `'list'` (Claude Code's built-in list prices), `'managed'` (the org's managed-settings `modelPricing` rates or multiplier — itself one of this bump's nine new settings keys), or `'unknown'`, which is the load-bearing value: no pricing row matched and no built-in price matched the model id, so `costUSD` is a GUESS at the default model's rate. Overwritten per request like `canonicalModel`, so a consumer differencing cumulative `costUSD` per turn gets that turn's basis; absent until this process has priced a request for the model (e.g. right after `--resume`) and on older builds, where it should be read as `'list'`. **Unprobed, zero consumers.** Directly relevant to the cost surfaces we already ship (`usage()`, the OTel cost metrics, the turn meter): it is the difference between reporting a number and reporting a number we know to be a guess |
