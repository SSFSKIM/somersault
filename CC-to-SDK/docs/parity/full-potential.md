...
| `PreModelSwitch` / `PostModelSwitch` hook events (0.3.251) | 🔬 | The one name-level drift of the 0.3.237→0.3.251 bump that matters: two new HOOK_EVENTS with cost-governance payloads (`from_model`/`to_model`/`requested_model`, `source`, `context_tokens`, `prompt_cache_warm`, `cache_ttl`, `estimated_cache_write_usd`, `pricing`). Pre returns a PreToolUse-shaped `permissionDecision` (allow skips the cache-miss confirm, deny cancels the switch, **ask refuses on a headless session** per its own doc); Post can inject `additionalContext` that rides the new model's first request. Source `sdk` covers headless `set_model` — so these plausibly DO fire headlessly, unlike most of the 22 dead hook events, but only a probe settles it (fire one via `Session.setModel`, both registration paths). Unconsumed; the product shape if alive is a model-switch budget-policy knob on Session/daemon |
| `SessionStart` resume-staleness fields (0.3.251: `seconds_since_last_response`, `context_tokens`, `prompt_cache_likely_expired`, `estimated_cache_write_usd`) | 🔬 | resume/fork-only payload additions on a hook we already pass through. Probe on `resumeSession()`/`forkSession()`; if alive they are free observability for the daemon's restart-resume path (supervisor restarts could report the re-cache cost they are about to pay) |
| `perTaskStopAffordance` + `stop_task` control (0.3.251) | 🔬 | New Options field + initialize declaration: a consumer that declares it renders per-task stop, and an interrupt on an open-input session then SPARES running background tasks (Stop aborts only the turn); absence fails closed (interrupt kills them). First-attached-client-wins; closed-input (`-p`) runs still kill hold-back tasks either way. Directly reshapes our daemon interrupt semantics — probe what the SDK transport actually honors headlessly, then decide whether ccx/daemon want to declare it and expose a `stopTask` op |
| SDK-MCP per-server `timeout` (0.3.251: `createSdkMcpServer` option + `McpSdkServerConfig.timeout` + initialize `sdkMcpServerConfigs`) | 🔬 | Knob-shaped: per-server tool-call wall-clock cap (≥1000ms, else falls through to `MCP_TOOL_TIMEOUT`), applied at first registration only. W4.1 pattern candidates: our 5 in-process servers (`cc-context`/`cc-compact`/tasks/swarm/permission) could each take a `timeout`. Probe or structural-passthrough judgment before wiring |
| result `queued_turn_count` + `user_message_uuid` echo on error results (0.3.251) | 🔬 | `queued_turn_count` on both result variants: pending user sends still queued when the result was produced (>0 = more turns follow without input; system entries not counted; absent on fatal-startup results). The success-path `user_message_uuid` echo is ALREADY consumed (M8, probes 94b/118b/119); NEW here are the error-result echo (join key for failure turns, no `request_sent_wall_ms`) and the queue-depth signal, which the daemon's turn loop could surface on `TurnOutcome` |
...
| background-task `ambient`/`is_backgrounded`/`spawn_depth` + reinit snapshot (0.3.251) | 🔬 | `task_started` gains `is_backgrounded` (a resumed subagent is always background) and `spawn_depth` (1 top-level, N+1 nested); `ambient` marks housekeeping tasks hosts should hide from activity indicators (flag flips re-emit the level signal); a repeated `initialize` now receives a background-task snapshot right behind the success response — the exact hole our daemon's reconnect story had (per-process level signal with nothing at startup). Probe the snapshot on a real re-initialize, then wire the daemon's task tracking |
| `hooks_applied` on repeated initialize (0.3.251) | 🔬 | Over stdio the CLI now re-registers the re-sent request's hooks on reconnect (`hooks_applied: true` from the stdin-owning process; false = ignored, absent = older CLI) and self-resolves hook callbacks it was still awaiting (denies a pending PreToolUse with a retry notice). Matters to the daemon restart/resume path and to any future multi-client host; probe alongside the reinit snapshot row above |
