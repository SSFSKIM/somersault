...
## 6. What was *not* wasted

A1's store, A2's bus/team/teammate runtime, and the permission/handshake design (A2b) are all real:
none are provided natively. The waste is narrow — A1's model-facing `cc-tasks` tool *surface* duplicates
native tools — and even that surface is the access path to the shared store the swarm needs.
