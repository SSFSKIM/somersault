...
>   approximated. **[Corrected 2026-08-13 by the fullscreen wave: the last two are no longer unreachable.
>   `ds()` was "fixed false" only because ccx had no alternate screen; it now boots into one, so the elapsed
>   suffix and the conjugation table are reachable-and-unbuilt and re-enter `tui-ux.md`'s denominator. The
>   wire-silence exclusions — Bash stdout, hook execution, the classifier — are untouched.]** **F4 then closed the static transcript** — everything the reader looks at once a turn is
...
> - **Fullscreen live-window wave (2026-08-13)** — ccx gains the renderer installed Claude Code actually
>   runs. Seventeen tasks plus a post-gate fix, spec
>   `docs/superpowers/specs/2026-08-12-fullscreen-live-window-design.md`; all **eleven acceptance cells
>   F1–F11 pass as written**, keyed and keyless, under an isolated tmux harness. What shipped: a complete
>   **alternate-screen renderer** — a fixed `rows - 1` frame, a virtualized painted-row-honest scrolling
>   viewport with canon's jump-pill grammar, the `v` transcript dump to `$EDITOR`, guarded
>   editor/`ctrl+z`/subprocess handoffs, canon's **two** overlay mechanisms (dialogs replace the dock;
>   pickers in a seam slot) with row budgets that hold by construction, the fullscreen surface deltas
>   (palette overlay, `statusLine` blank-row semantics, an immediate-notice home in the footer), a live
>   `/tui default|fullscreen` switch that **never unmounts the conversation**, and **fullscreen as the
>   default** behind a ten-rung selection ladder (env pins, settings, a tmux `-CC` probe, screen-reader and
>   non-TTY floors). The resize matrix grew a fullscreen cell (`f1`) and now runs **10 keyless / 11 keyed**.
>   **No domain score below moves, and that is the accurate result:** the wave consumes **no SDK surface at
>   all** — it is terminal bytes, Ink layout and ccx's own settings; every engine lever in the range
>   (`getContextUsage`, the frame stream, `applyFlagSettings`) was already modeled. Two Ink-level facts worth
>   carrying because any embedder rendering its own frame over this SDK inherits them: Ink keeps **one**
>   log-update whose `previousLineCount` never resets across a screen change (a leave edge otherwise spends
>   the alt-screen frame's erase on the user's real shell), and Ink caches the `<Static>` box's yoga node on
>   its root and never clears it, so a tree that unmounts Static reads freed WASM memory on every later frame
>   — an intermittent crash whose fix is to keep the Static owner mounted and empty. TUI/UX parity: see
>   `tui-ux.md`'s fullscreen-wave recount — **overall flat at ~75%**, because building the renderer
...
> - **Tool-stream wave (2026-08-19)** — the fullscreen renderer's tool cluster: adjacent tool calls fold
>   into one dim summary row (`Searched for 1 pattern, read 2 files, ran 2 shell commands`) with all Bash
>   collapsing, the task-board tools and `ToolSearch` absorbed silently (popping out standalone on error),
>   git operations scraped from bash output into their own clauses, and the `(ctrl+o to expand)` chip
>   suppressed everywhere fullscreen paints; while the turn runs the row is live (spinner, present-tense
>   verbs, a `⎿` hint line, watermarked counts, an elapsed `· 2s` after two seconds); and **clicking a
>   cluster expands it in place** into its members' per-call rows — a full mouse path from SGR byte decode
>   through a sink registry and a published row map to expansion state. The classic renderer is unchanged.
>   Thirteen tasks plus one re-scoped repair, a whole-branch external review and two acceptance runs;
>   **acceptance is 10 PASS / 1 partial / 0 FAIL**, split honestly into live-verified cells (A1, A3, A5's
>   ctrl+o round trip, A11, and A10 in part) and replay-verified cells driven over a real pty from staged
>   session JSONL (A2, A4, A6, A7, A8, A9). Gates at close: typecheck clean, tui 3748 (9 live-skips), unit
>   2891. Spec `docs/superpowers/specs/2026-08-18-tool-stream-design.md`.
>   **No domain score below moves, and that is the accurate result** — like the fullscreen wave this is
>   terminal bytes, Ink layout and pure projection code; it consumes no SDK method, option or frame this
>   table did not already claim. The parity it buys is scored in `tui-ux.md`'s tool-stream recount (§2 rises
>   as the cluster and click rows leave ❌ for 🟡, and one row is marked DOWN — see the OSC-8 note below).
>   **Three SDK-level facts came out of it, all live-probed or live-measured, and all worth carrying
>   because any embedder inherits them:** (1) **no per-tool progress feed is reachable headlessly** — probe
>   `probes/probes/100-tool-progress-stream.ts` (SDK 0.3.220) saw zero `tool_progress` frames between a
>   tool's `tool_use` and its `tool_result` in the default environment and no output-line count on any
>   frame (eleven spellings scanned); the single elapsed carrier, `tool_progress.elapsed_time_seconds`,
>   appears only under `CLAUDE_CODE_REMOTE`/`CLAUDE_CODE_CONTAINER_ID` and only once per 30 s per call, and
>   its `tool_use_id` is a synthetic producer id (`"bash-progress-0"`) with the real id in
>   `parent_tool_use_id`. So a UI that wants elapsed time must keep its own clock, and a UI that wants an
>   output-line count cannot have one. (2) **A parallel tool batch's results are delivered together, in
>   issue order** — measured live: a fast Read issued alongside a slow Bash had its `tool_result` land
>   13–26 ms *after* the Bash's, ~20 s after the read itself must have finished. Arrival order therefore
>   equals issue order, never completion order, which is why the external review's result-reordering
>   scenario is real in our code and unreachable through today's transport (that acceptance cell is the
>   recorded **partial**, and its fix is kept as defensive rather than load-bearing). (3) our wire carries
...
>   its exposure, since expanded clusters put many file rows on the main frame.
>   **Probe-number hygiene:** this wave's `100-tool-progress-stream.ts` collides with Wave C's
>   `100-prompt-suggestion-and-spinner-tokens.ts`, as `106` and `109` already collide from earlier waves —
...
| 10 | **Remote / bridge / voice / UI** — `connectRemoteControl`, remote server, voice, Ink TUI | ~76%¹ | ✅/🚫 | `bridge/` control-protocol shim built; **Phase-3 increment 1 SHIPPED — `cc-harness top`, a read-only terminal daemon-observability dashboard** (lightweight, no-Ink `monitor/`: polls `list`+`context_usage`, renders the live pool / ctx% / token usage / proactive heartbeat with idempotent teardown; CLI-internal, public surface unchanged — spec/plan `2026-06-18-daemon-observability-dashboard`). **Phase-3 increment 2 SHIPPED — `cc-harness-console`, an interactive Ink daemon console** (new `cc-harness-tui` package over the core's new public `connectDaemon`/`DaemonClient`: master-detail pool/detail, inject prompts via streaming `submit`, drive control ops — interrupt/setModel/setPermissionMode/compact/fork/proactive — with confirm-gated `stop`; spec/plan `2026-06-18-interactive-daemon-console`). **Phase-3 increment 3 SHIPPED — `cc-harness-chat`, an in-process chat REPL** (in `cc-harness-tui` alongside the console bin: drives a live `openSession`/`Session` in `default` permission mode with rich tool rendering via `render.ts` — bespoke Edit/Write/Bash/Read formatters — and inline permission dialogs via `createPermissionGate`/`PermissionBroker` advanced-seam wired through `uiBroker.ts`/`useChat.ts`; spec/plan `2026-06-19-chat-repl-permission-prompts`). **Phase-3 increment 4 SHIPPED — daemon-attached interactive permissions** (auto-autonomy + poll-based escape-hatch wire: `PendingPermissions` registry parks `default`-mode edit requests via `createPermissionGate`; `DaemonSupervisor.pendingPermissions()`/`respondPermission()` expose the poll/answer surface; clients poll the snapshot `pending` list and answer via `DaemonClient.respondPermission()`; auto-mode sessions bypass the broker entirely — no pending ever queued; gated live e2e confirms both paths; spec/plan `2026-06-19-daemon-permissions`). **Phase-3 increment 5 SHIPPED — live streaming + live status bar** (`cc-harness-chat`): a pure `tui/src/liveTurn.ts` reducer turns SDK partial `stream_event` frames into live token-by-token text, stream-then-collapse thinking (`✦ Thinking`), and in-place tool `⟳ running → ✓/✗ done` status; the status bar shows live model · mode · ctx% · `⟳ streaming`. Engine change is one flag (`includePartialMessages`) — no harness source change. Probe 20 verified partials flow in the multi-turn streaming-input Session path; spec/plan `2026-06-19-chat-live-streaming`. **Phase-3 increment 6 SHIPPED — slash commands** (`cc-harness-chat`): `/model /compact /context /clear /help /resume` intercepted locally (probe 21: the SDK gates /model//help//resume "not available" headless) and dispatched to engine ops already built — `setModel`, `Session.compact`, `getContextUsage`/`summarizeUsage`, `listSessions`/`resumeSession`. A pure `commands.ts` (parser + table + formatters) + a `SessionPicker` modal; `useChat` is now factory-owned so `/resume` swaps the session (marker+continue). No new harness exports; spec/plan `2026-06-19-chat-slash-commands`. **Phase-3 increment 7 SHIPPED — rich tool rendering + display-G** (`cc-harness-chat`): subagent (`Agent`) turns nest under their parent via `parent_tool_use_id` and collapse-on-done (`⚙ Agent … ✓ (N tools · Ts)`); a pinned `TaskPanel` live-reduces native `TaskCreate`/`TaskUpdate` ops (probe 22b: the SDK has no `TodoWrite`); inline Edit/Write diffs; a status-bar subagent indicator. Two pure reducers (`liveTurn` extended, new `taskList`) + a clock injection; one flag (`forwardSubagentText`, already plumbed) — no harness change. Probes 22/22b verified nesting reachable headless; spec/plan `2026-06-19-chat-rich-tool-rendering`. **Phase-3 increment 8 SHIPPED — input ergonomics** (`cc-harness-chat`): a new multiline editor replaces the single-line input — `\`-continuation + multi-line paste (probe `ink-paste-key-delivery`: a paste is one `useInput` call), in-memory prompt history (Up/Down recall with draft stash/restore), and `@`-mention fuzzy file completion (recursive walk with basic ignores, path-token insert). A pure `editor.ts` reducer + a pure `fileComplete.ts` ranker (injected `readdir`) + a thin `ChatComposer.tsx`; the shared console `<Composer>` is untouched. No harness change; spec/plan `2026-06-19-chat-input-ergonomics`. **Phase-3 increment 9 SHIPPED — session resume/continue** (`cc-harness-chat`): resume now *renders the conversation you're rejoining* — launch `--resume <id>` / `--continue` (most-recent), a `/continue` command, and the incr-6 `/resume` picker all converge on one `resumeInto` that fetches `getSessionMessages` first (empty → notice, no swap) then replays the transcript full-fidelity. A pure `replay.ts` reuses `render.ts` (`renderMessage` promoted to render prompts + delegate Edit/Write diffs to the shared `toolDiffLines`), skips tool_result bodies, caps the last ~200 messages, and frames the block with resumed/live dividers. Probe 23 verified the persisted message shape = the live shape; no harness change; spec/plan `2026-06-20-session-resume-continue`. **Phase-3 increment 10 SHIPPED — mature `auto` + graceful permission ladder** (`cc-harness-chat`): `Tab` now cycles `default → acceptEdits → auto` (bypass gated behind `/yolo` + `--permission-mode`); the `auto` rung self-heals the model live (`setModel` to a supported model + a notice) since `auto` is the model-gated headless classifier (probe 24: it bypasses the broker entirely — no inline dialog for safe ops, and actively blocks dangerous ones). The `auto` model-gate is now centralized in `resolveOptions` (every lib/`createHarness` caller is born auto-safe, not just the daemon), `resolveAutoModel`/`isAutoSupportedModel` are exported (advanced-seam), and the console's `cyclePermissionMode→auto` issues a `setModel` repair op. Probe 24 verified the lib-seam auto behavior + runtime enable/repair; spec/plan `2026-06-20-auto-permission-ladder`. **Phase-3 increment 11 SHIPPED — interactive thinking-budget control** (`cc-harness-chat` + console): a `/think <off|low|medium|high|xhigh|max|N>` command sets the extended-thinking budget at runtime via the already-built `Session.setMaxThinkingTokens` lever (probe 25: it takes effect mid-session and `0` disables thinking, which is ON by default), a `--think <level>` launch flag opens at a baseline budget (`thinking` config), the status bar shows `think:<level>`, and the daemon console gains a `cycleThinking` (`t` key) issuing the existing `set_thinking` control frame. The level vocabulary borrows the SDK effort enum; the mechanism is the thinking token budget. A pure `thinkLevels.ts` is the single source of truth — NO harness change (every lever was already built). Spec/plan `2026-06-20-thinking-budget-control`. **Increment B SHIPPED — chat REPL UX fidelity** (`cc-harness-chat`): three CC-faithfulness gaps closed — (#11) **markdown rendering** via a pure `markdown.ts` (`renderMarkdown` → styled `RenderLine[]`: whole-line bold/italic/inline-code, headers, bullet/numbered lists, fenced code dim+indented, blockquotes; mixed-style line strips markers as the accepted lightweight-parser limit) wired into the assistant + live *text* branches only (thinking/user/tool lines stay raw — and resumed transcripts inherit it free via `replay.ts`'s `renderMessage` reuse); (#10) a **`ThinkingIndicator`** spinner + elapsed shown in the transcript the instant you submit (one interval mounted only during the `busy && streaming-empty` gap); (#8) a **`/model` picker** modal (mirrors `SessionPicker`) fed by the *live* `session.capabilities().models` (probe 27: `supportedModels()` returns a rich 6-model list headless — flipping the dashboard m-bug's "empty headless" trace), with the free-text `/model <name>` fast-path preserved. Entirely `tui/` — NO harness change. Spec/plan `2026-06-20-chat-repl-ux-fidelity`. **Increment C SHIPPED — dashboard live state** (`cc-harness-console`): the daemon console now *mirrors live per-session state* by closing the control→display loop — (#6) **model cycling fixed** — the real root cause was `modelId` reading `.id`/`.model` (→ `set_model="[object Object]"`); now maps the SDK objects' `.value` (the audit's "supportedModels empty headless" trace was DISPROVED by probe 29: the harness `Session` returns the 6-model list pre-turn because `readLoop` primes the control channel in the constructor) — and the cycle is now *visible* via a daemon **source-of-truth write-back**: `supervisor.control()` writes the new model/permissionMode back to the registry record + `configs` on a successful `set_model`/`set_permission_mode`, so `list()→collect()→SessionRow` reflect live state for any client (surviving reconnect); (#2) **per-session permission mode** carried `SessionRecord`→`ListEntry`→`SessionRow` and shown in Detail; (#4) **enriched Detail** — a dim `mode · ctx% · tokens · age · proactive` line; (#5) **proactive glyph** in Pool (`▶` running / `⏸` paused). Daemon source-of-truth chosen over console-display-only; basic proactive state (no new daemon API). Gated live e2e proves the write-back end-to-end (GREEN under OAuth subscription auth — probe 28). All new fields optional → backward-compatible. Probes 28/29 ground it; spec/plan `2026-06-20-dashboard-live-state`. **Increment D SHIPPED — command palette** (`cc-harness-chat`): the chat REPL now surfaces the live slash-command catalog as an inline `/`-autocomplete and dispatches it (capability gain logged under domain 8). A1 triple-flip recorded: #13 was a *surfacing* problem, not installation — the 105-command catalog (skills + plugin + user commands) is already loaded (probe 30, inherited via `settingSources`) and already invokable (probe 31: built-ins gate, but skills/plugin/user commands execute as prompts); the REPL just discarded `capabilities().commands`. Mirrors the `@`-mention mechanism: a pure `commandComplete.ts` (catalog entry type + `mergeCommands` local-wins + `rankCommands` reusing the `fileComplete` fuzzy scorer), a `command` state in `editor.ts` parallel to `mention` (mutually exclusive, command-first in every shared branch; `/` opens only at buffer start; **Enter completes-AND-submits `/name`** — the deliberate divergence from `acceptMention`; Tab completes the name + space; Esc/space/cursor-leave close), a `CommandPopup` in `ChatComposer` (Box-row, live-injected catalog, `argumentHint` shown), and 3-way dispatch routing in `useChat` (LOCAL→engine switch / CATALOG→submit-as-prompt / unknown→`formatUnknown`; the `/zzz`-unknown regression preserved). Entirely `tui/` — NO harness change (`Session.capabilities()` already existed). 9th ZERO-PER-TASK-FIX-CYCLE increment (5/5 tasks clean on first review; one post-final-review polish commit = `argumentHint` render aligning to spec D3 + a close-path test). Keyed-OAuth live e2e GREEN (catalog non-empty headless, 8s). Probes 30/31 ground it; spec/plan `2026-06-20-command-palette`. **Agent app-server M1 SHIPPED (2026-07-29)** — a new JSON-RPC control plane (`harness/src/appserver/`) over WebSocket (`ccx serve`) so a future web UI can drive a real `Session` remotely: `initialize`/`thread/*`/`turn/*`/`decision/*`/`thread/subscribe`+`thread/read` plus `item/*`/`turn/*`/`decision/*` notifications, built on **decisions-as-state** (a permission/question/plan request parks; any client can `decision/respond` later — no reverse-RPC). Live-accepted end to end (spawn→subscribe→turn→park→respond→completed, a real Bash permission park over a real WS connection, `harness/test/live/appserver-m1.test.ts`). Seam-coverage scorecard (generated from the four real seam sources, not hand-counted) at `docs/parity/appserver.md`; plan `docs/superpowers/plans/2026-07-28-agent-appserver-m1-core-loop.md`. **Wave T (2026-08-06) — score UNCHANGED at ~66%; two corrections to the text above.** (1) Increment 10's "the `auto` rung self-heals the model live … since `auto` is the model-gated headless classifier" is right about the repair and **wrong about what happens without it**: probe 99 shows the engine *refuses* the mode rather than degrading it, so an unrepaired `auto` request leaves the session where it was — which is why the plan applier now swaps first and why a refused change is reported rather than painted. (2) Increment 3's "drives a live `openSession`/`Session` in `default` permission mode" is true again for the interactive REPL, but only because Wave T restored it: Increment A had made every caller `auto`, and the QA fleet found a fresh `ccx` executing `rm` unconsulted as a result. Wave T also added the app-server's share of the plan-grant work (`planUpgrade.ts` applies the mode the approval *granted*, not a hard-coded `acceptEdits`), and gave the REPL a live retry indicator driven by `system/api_retry` frames that were already arriving unrecognised. No new SDK surface is consumed, so the fraction does not move. **Agent app-server M2a + M2b SHIPPED (2026-08-11) — raised ~66% → ~72%**, the first move in this domain since M1, and it is realized envelope rather than re-characterization: M1's core loop is now a complete control plane over an in-process SDK engine — **51 registered JSON-RPC methods and 26 notifications** (`harness/src/appserver/`) covering settings + introspection, the session library (`thread/list|read|fork|name/set|tag/set|delete`), lifecycle (`thread/reinitialize|close`), the rewind trio, the MCP quintet, background tasks, the nine settings-ops methods the host wire had grown past the spec's 25-op inventory (scorecard gap 6), a server-side turn queue with enqueue-minted ids and a closing latch, and three probe-promoted seams — `turn/steer` over `streamInput`, `plugin/reload`, `skill/reload` (probes 103b/105; probe 104 retired `readFile` as **dead** — resolves null for existing and missing paths alike, so M3's `fs/read` cannot back onto it). Every method is zod-typed with generated draft-7 JSON-Schema artifacts (`--emit-schema`) and re-exported at the `cc-harness/appserver` subpath, and the seam scorecard is machine-gated in three directions — presence, status staleness, and method↔row bijection — so a shipped-vs-planned lie is a red build rather than doc rot (the M2a lesson: 15 methods once shipped while their rows read `planned(M2)` and the gate stayed green). **The acceptance proved the plane against a real engine, not fakes**: one keyed live run performed the spec's full sequence — 14/14 assertions in 48s — including two-client write-back fan-out, a decision park and respond, rewind anchors + `dryRun`, queue enqueue→drain, a steer whose result correlated to the *prompt's* uuid, compact with an outcome, a fork sharing item ids with its parent, `thread/clear`, and a clean shutdown leaving zero registry entries; a zero-dependency HTML console (a deliberately foreign consumer) then drove every waves-3–4 panel live with zero runtime errors. **What holds the domain below higher is M3, and it is named**: fleet adoption (`thread/attach`/`thread/stop` — the only non-shipped row on the scorecard's 82; until it lands, a fleet-origin thread's `-33006` is defined-but-unemitted and the host-wire/bridge seams stay unimported), the workspace surfaces `fs/*` and `shellCommand`, and a re-open path for the one known residual (a factory throw inside the engine swap leaves the record holding a disposed engine — scorecard gap 10, reads honestly as `-33005` but has no recovery short of close + resume). Spec `docs/superpowers/specs/2026-07-30-agent-appserver-m2-design.md`; plans `…-m2a-spine-controls.md` / `…-m2b-rewind-queue.md`; scorecard `docs/parity/appserver.md`. **QA wave-2 delta (2026-08-11) — score UNCHANGED at ~72%.** Ten tasks of chat-REPL repair (see the wave entry above and `tui-ux.md`'s wave-2 recount) consuming no new SDK surface: every lever they use — the `canUseTool` deny arm, `applyFlagSettings`, `getContextUsage`, the hook channel, the session reader's message array — was already modeled. The one thing worth carrying here is a *deployment* fact the wave forced: because ccx always passes `canUseTool` and permission mode is runtime-mutable while the callback is construction-only, the SDK's `CLAUDE_SDK_CAN_USE_TOOL_SHADOWED` warning fires on every bypass launch and used to print raw over the Ink frame. Dropping the callback in bypass is **not** safe — a bypass launch would be permanently brokerless after a runtime step-down — so the fix is a warning-channel takeover at the CLI entry (SDK-coded warnings to a debug seam, everything else re-printed once above the frame). Any embedder rendering its own frame over this SDK inherits the same problem **Agent app-server M3 SHIPPED (2026-08-12) — raised ~72% → ~76%**: the app server becomes a **fleet surface** — it attaches to the running ccx background fleet (per-session detached hosts, one UDS each) so a browser console or IDE can list, adopt, drive, and stop sessions it did not spawn. **58 registered methods / 26 notifications** now, up seven: `fleet/list` + `thread/attach` (adoption with a concurrency reservation + a buffered-events activation barrier), `thread/stop` (EOF-is-the-contract, not a receipt — the host tears sockets down before any handler could reply), the workspace pair `fs/read` + `fs/search` (server-scoped, Codex-parity trusted-client), `thread/shellCommand` (display-only exec in the thread's cwd — output returns to the calling client, the model never sees it), and `thread/reopen` (gap-10 recovery — a dead-engine thread swaps to a resumed or fresh replacement instead of being permanently `-33005`). The heart is a **second structural `EngineSession`** (`appserver/fleetEngine.ts`) that drives a live host over its socket, and a fleet event layer that is the **sole owner of turn lifecycle** for own and foreign turns alike — so every capability the M2 in-process plane exposed (turns, decisions, rewind/clear, MCP, settings, tasks) now reaches an adopted thread by forwarding the host's own op, with **zero interim hazards** (every path that once ran an in-process swap on a fleet record now forwards or is origin-refused `-33006`, verified by an independent re-derivation). Six additive host-wire revisions carry it (turn-end now ships the turn's `result` and its soft-failure `TurnFailure` tag — a live probe proved no message frame ever carries a result to a follower). **Acceptance was a keyed live run against a real spawned ccx host, not fakes**: 13/13 legs in 35s — attach into a busy foreign turn with replay-burst-before-reply ordering, a permission park answered from a second WS client with attribution + the structured answer, a wrong-kind answer mapped `-32602`, a foreign `clear` on the raw host socket announcing `thread/rewound` in 442ms (679× under the 300s swap cap), transcript paging, the workspace pair, shell in the session cwd, detach + re-attach (host survives), stop to a terminal roster, and an inProcess reopen leg that SIGKILLs a live engine mid-park (the park settles at death time via the SDK's own abort, so reopen recovers a clean slate — a live-only truth the fake could not produce); a zero-dependency HTML console then drove the fleet + workspace panels live with zero JS errors. Sixteen SDD tasks, each through implement→independent-review→fix→re-review; the review layer caught five request-never-settles hangs and two Criticals that no task's own tests saw. Spec `docs/superpowers/specs/2026-08-11-agent-appserver-m3-design.md`; plan `…-m3-fleet-workspace.md`; scorecard `docs/parity/appserver.md` (88 rows, 58 methods, machine-gated). **Fullscreen live-window wave (2026-08-13) — score UNCHANGED at ~76%, and the reason is structural rather than a judgement call: the wave is entirely renderer-side.** ccx now ships an alternate-screen renderer as its default (fixed `rows - 1` frame, virtualized painted-row-honest viewport, canon's two overlay mechanisms, a live `/tui` switch, a ten-rung selection ladder; eleven acceptance cells F1-F11 pass as written, resize matrix 10 keyless / 11 keyed with a new `f1` fullscreen cell) — but every byte of it is terminal control sequences, Ink layout and ccx's own preference file. No SDK method, option or frame is consumed that this table did not already claim, so nothing here moves; the parity it buys is scored in `tui-ux.md`, which stays flat at ~75% because the renderer also made six previously-excluded fullscreen surfaces reachable. The one thing worth recording at SDK level is an **Ink** constraint any embedder that paints its own frame over this SDK will hit: Ink keeps a single log-update whose `previousLineCount` survives a screen change, and it caches the `<Static>` box's yoga node on the root without ever clearing it — unmounting Static therefore reads freed WASM memory on every subsequent frame. **Agent app-server M4 SHIPPED (2026-08-16) — score UNCHANGED at ~76%, and the reason is the point.** M4 adopts Codex's **code-review domain**: **59 registered methods / 27 notifications** now, up one each — `review/start {threadId, target, delivery?}` → `{turn, reviewThreadId}` (Codex's shape verbatim, all four target variants) and the `review/findings` notification. It moves no fraction because it consumes **no new SDK surface**: a review is an *ordinary turn on a new thread*, not a child session with re-stamped events the way Codex's is, and the findings ride a tool the engine already has. That last part is also where we come out **ahead of what we ported** — Codex flattens its findings to a string for the model to read, we ship the array (`file`, `line`, `summary`, `short_summary`, `failure_scenario`, `category`, `verdict`, `outcome`), which is affordable only because the native `ReportFindings` contract exists. Probe 109 settled every premise live on 0.3.227 before any of it was designed: the tool is present and callable in a plain headless session, the model calls it off a bare instruction with no review-specific system prompt, and the payload rides `tool_use.input` matching the declared shape. The honesty rule is on the wire, not in a comment: a review turn that ends with **no** `ReportFindings` call publishes `findings: []` **plus** `unstructured: true` **plus** the reviewer's own prose — never a bare empty array, which would be an all-clear no reviewer asserted — and **plus `aborted: true` when the turn was interrupted or failed**, since an interrupt ends on a real terminal frame and would otherwise render as "no defects found"; a turn that died before any terminal frame publishes nothing at all and lets `turn/completed {status:"failed"}` say so. The review thread inherits the target's config re-rooted at its cwd, with **all six session-identity knobs** dropped — `resume`, `sessionId`, `continueSession`, `resumeAt`, `droppedTurnUuid`, `forkSession`, in this repo's vocabulary and again in the SDK's, since `extraOptions` reaches `Options` without passing a typed field — else "detached" would run on the target's own transcript; and the edit tools denied. The final review found that neither the re-rooted `cwd` nor the denials were actually authoritative, because `resolveOptions` spread the client's `extraOptions` escape hatch **last**: four keys (`cwd`, `disallowedTools`, `canUseTool`, `onElicitation`) are now re-asserted after that merge, and only where the server computed one, so the hatch still wins wherever it defeats nothing (D-M4-11). Even so, the read-only posture is **risk reduction, not a guarantee**, and the two open doors are named rather than implied — `Bash` (a review needs git) and namespaced MCP write tools — with subagents proven NOT to be a third (probe 110: a top-level `disallowedTools` binds a dispatched child at depth 2 as well as 1, and `bypassPermissions` does not lift it). `delivery:"inline"` is refused with an actionable error naming detached (D-M4-2: Codex splices a child session's events onto the parent turn by re-stamping ids, which the SDK gives us no way to do). **The same milestone made MCP elicitation reachable end to end** — an MCP server's request for user input now parks as a **fourth decision kind** beside permission/question/plan, listed by `decision/list` and answered by `decision/respond` with `elicitation_accept`/`_decline`/`_cancel` (MCP's own action enum), fail-closed by construction so no failure path leaves a server hanging on a response that never comes. That is the one seam here the envelope already counted — `onElicitation` has been a modeled Options field since Wave 4 — so it upgrades a *modeled* item to a *proven-reachable* one rather than adding a numerator entry, which is why the fraction holds. Spec `docs/superpowers/specs/2026-08-13-agent-appserver-m4-review-design.md`; scorecard `docs/parity/appserver.md` (90 rows, 59 methods, machine-gated — and note that the gate could not have caught this landing's omissions: it checks that walked tokens have rows and that shipped rows name real code, so an unregistered, unrowed method is invisible to it; the count moving 58→59 and 88→90 is the evidence). **Tool-stream wave (2026-08-19) — score UNCHANGED at ~76%, for the same structural reason as the fullscreen wave: it is renderer-side.** The fullscreen tool cluster, its live dressing and its click-to-expand pipeline are terminal bytes, Ink layout and pure projection code over frames this table already claimed (see the wave entry above for the acceptance matrix — 10 PASS / 1 partial / 0 FAIL — and the three transport facts it produced: no headless per-tool progress feed, parallel tool results delivered together in issue order, and no message timestamps on our wire). **Agent app-server M5 SHIPPED (2026-08-19) — score UNCHANGED at ~76%, but NOT for M4's reason: M5 is the first milestone since M2b to consume new SDK surface and still not move this number.** M5 is the *configuration and library* milestone: **66 registered methods / 29 notifications** now, up seven methods and two channels. The **config trio** reads and writes the settings-FILES chain (user < project < local < managed) with upstream's own merge semantics — `config/read {includeLayers?, cwd?}` returns the effective merge, dotted-leaf `origins` naming the layer that won each leaf (every contributor, for an array leaf), and `versions`, the per-target CAS token without which a client facing an existing file could make nothing but a last-wins first write; `config/value/write` + `config/batchWrite` take `keyPath` as an opaque segment ARRAY rather than Codex's quoted-dotted grammar (which dissolves the escaping problem outright, at the price of refusing `__proto__`/`constructor`/`prototype` as a segment and as a key anywhere inside a written value), and make the CAS check **atomic with the write** rather than advisory — a per-file queue holding a nonce-owned `O_EXCL` lockfile across read → validate → tmp → rename — so two writers carrying the same token serialize and exactly one commits, the loser refusing `ConfigVersionConflict`. The engine's *applied* view is deliberately left where it lives (`thread/settings/read`), and `config/read` declares its own shortfall (`incomplete`) rather than implying it covers non-file policy sources. **Search** is two methods that differ in kind: `thread/search` scans every session on this machine — metadata and persisted transcript alike — on a keyset cursor `(sortValue, sessionId, rowIndex)` whose caps bound work per request and never coverage (a page may legitimately return zero matches with a non-null `nextCursor`, oversize rows are disclosed in `skipped`, and a store read that FAILS refuses the request instead of answering zero — "no matches" is a claim about content actually scanned); `thread/searchOccurrences` narrows to one thread, returns every hit in it over exactly the corpus the session-row classifier defines (so search and replay cannot drift), and hands each live-thread occurrence a `readCursor` that `thread/read` accepts unchanged, making "find it" and "go there" one round trip. **Archive** is server-owned marker files (`~/.claude/ccx/archived/<sessionId>`): one atomic create, one unlink, no read-modify-write to race, idempotent in both directions, refusing to shelve a live thread — where "live" includes a resume reservation, re-checked after the marker is written — and auto-unarchiving from every admission surface (`thread/resume`, a resume-carrying `thread/start`, `thread/attach`), since opening a conversation takes it off the shelf. It reaches clients through the milestone's one change to an existing method, additive: `thread/list {archived}`. **That much of it — the config trio, both searches, the archive pair — moves no fraction because it consumes no new SDK surface** (the absorb task below is the part that does) — settings are node's own fs plus a mirror of upstream's merge customizer, search stands on the `listSessions`/`getSessionMessages` readers this table has counted since domain 6, and archived-ness does not exist upstream at all, which is why nothing walks it. What it does add is schema depth: every one of the seven methods publishes its **response** schema beside its request one (`MethodSchema` gained a `result` slot for them, D-M5-19), and the three disk/sidecar readers join `ENGINE_GONE_EXEMPT` so a dead-engine thread stays searchable and shelvable by its own id. **The absorb task then consumed two new 0.3.234 surfaces, and this cell says so because Task 11 had written the opposite.** Task 11 held this domain at ~76% on the explicit reasoning that M5 consumes no new SDK surface; probes 111 and 112 came back ALIVE on both origins and Task 13 wired both, so that sentence is now false and is replaced here. What ships: an assistant frame's wrapper-level `context_usage` — 0.3.234's structured twin of the `/context` report — forwarded verbatim as an optional `contextUsage` sibling on that frame's own `item/started`/`item/completed` notifications, with no retention and `thread/contextUsage/read` deliberately untouched (that read serves `getContextUsage()`, which is richer and costs no turn, where the twin costs a `/context` turn — D-M5-22 amended the spec's original promote sentence rather than following it); and the init frame's `terminal_slash_commands`, latched by its OWN frame route onto the thread record and served as an optional `terminalSlashCommands` field of `thread/capabilities/read`, which is the one place a remote or web UI can learn that a command it was offered only runs in a terminal — `supportedCommands()` carries no such marker, so the init frame is the only source in the whole SDK surface. **Two message-level fields are real capability and still do not move a domain fraction**, which is the honest reading rather than a rounding dodge: the numerator here counts surfaces turned into capability, and two optional fields on messages this table has consumed since M1 do not reach a percentage point against the reachable sub-surface footnote 1 describes. **Acceptance was a keyed live run against a real engine, not fakes**: 9/9 legs in 10.3s — the config chain end to end against a temp settings home (deep merge, leaf origins, CAS tokens, a stale-token conflict, a masked write); a real engine's init frame reaching `thread/capabilities/read` as `terminalSlashCommands`; a `/context` turn carrying `contextUsage` on `item/started` AND `item/completed`; a marker planted in a LATER transcript row of a closed session found by `thread/search` over the real store with a snippet (no metadata field can carry it, so finding it is a claim about a transcript actually being read); every returned `readCursor` fed back to `thread/read` UNCHANGED landing a window whose last row holds the match; archiving a LIVE thread refused BUSY by either spelling of its id; the archive round-trip hidden, revealed, broadcast to a second client and unshelved by a resume; and a safety leg asserting the operator's own `~/.claude` never entered a run that ships config WRITES — plus M4's acceptance re-run 7/7 in 122s as the regression check that config/search/archive left the review domain alone. **The leg worth naming is the one written to survive its own unprovability**: it tees the raw `system/init` frames and asserts against whatever the engine sent, then PRINTS what it saw — `3 system/init frames; terminal_slash_commands present on 3/3; last value = ["doctor","color"]` — which settles empirically that **init is re-emitted per turn** (the fact the fleet origin depends on, since an attaching client's replayed burst is dropped) and that this CLI never omits the field, so D-M5-22's absent-key branch **cannot be staged here** and stays contract-derived from the SDK's own doc comment rather than observed. Recorded with its own trap, because it was nearly shipped dead: the latch had to be its own route and not a branch inside the session-id latch, which early-returns on a stamped `record.sessionId` — and a fleet thread gets that id from its host's `state` event, so the guard is already closed before its first init frame arrives. What saves the fleet origin at all is that init is re-emitted every turn; a once-only init would have been in-process only, since an attaching client's replayed burst is dropped by the router by design. Spec `docs/superpowers/specs/2026-08-18-agent-appserver-m5-config-search-design.md`. **M9 — ARRIVAL HISTORY SHIPPED (2026-08-30)**: an inbound cross-session message now survives into `thread/read`. The CLI persists an arrival as an `isMeta` row and the SDK's `getSessionMessages` returns no `isMeta` row at all, so a client reading its own history saw the answer and never the question — the absence M8's LEG 2 was written to assert, and the presence it asserts now that the milestone closed it. The server LOGS each arrival as it observes it (an append-only per-session sidecar, one file per entry, anchored to the last user/assistant frame with that row's predecessor and a content hash) and projects the entries back as `origin`-marked `userMessage` items at their anchors on every read, with the matching anchored scan in `thread/searchOccurrences` so the text is findable and its `readCursor` lands. **No domain score moves, for M4's and M5's reason** — this consumes no new SDK surface; it repairs a defect in this server's own history reading with a sidecar the SDK knows nothing about, and because an arrival rides its anchor row rather than occupying one, the published cursor pattern is byte-identical in the generated artifact and the only wire change is two additive optional response members. The limits are stated rather than implied: an arrival landing before the engine reveals a session id is announced and never logged; a write failure degrades loudly (`arrivals: null`) and durably wherever a marker can still be written, in-process only where the fault is the directory itself; an anchor that cannot be resolved withholds its item instead of misplacing it, with the omission visible because `logged` exceeds the marked items returned; and per-message identity inside a batch the engine collapsed does not exist in the data at all — one frame, one entry, both texts under one uuid. Gates at the landing: unit **4285 tests / 274 files** green, typecheck clean, the appserver scorecard gate unmoved (110 rows / 73 methods), and a keyed cross-session acceptance **10/10 in 166s** against a real engine — including the flipped LEG 2 and a `limit: 1` walk over a batched exchange that terminates without stranding an arrival. Spec `docs/superpowers/specs/2026-08-29-agent-appserver-m9-arrival-history-design.md`. Remote/voice remain 🚫/non-goal **by design** |
...
| Hook events (`HOOK_EVENTS`) | 30 declared by the SDK; **33 in the engine's own dispatcher registry** | first-class `config.hooks` + 4 builders + `mergeHooks` | **24 verified-fired headlessly** (23 re-measured 2026-09-01 over the whole registry, `reforge/w5/probe-hook-events.ts` + `reforge/research/fixtures/hook-registry-2.1.251.json`, pin 2.1.251; `PermissionDenied` joined them 2026-09-02, below). The 2026-06 count of 8 measured CALLBACKS only, on one turn that created none of the missing conditions; the 12 that replaced it still watched a hand-written list, so three live events were never watched at all. **SessionStart and SessionEnd are NOT dormant** — SessionStart fires on the settings/command path only, because its dispatch precedes host-hook registration. The other 10 registry events are **OPEN** with named conditions, not ruled out. **W6/C9 closed one of them with a run**: `PermissionDenied` was first probed by creating an ordinary broker denial with both hook paths armed — `PermissionRequest` fired, `PermissionDenied` stayed silent, which was the right answer to the wrong condition, since its sole dispatch site is guarded on `decisionReason.type === "classifier"`. Its real condition was then created rather than waited for (2026-09-02): under `auto`, with the classifier's own API call made unavailable, upstream denies fail-closed with `{type:"classifier", classifier:"auto-mode"}` and **`PermissionDenied` FIRED on both hook paths** (`reforge/w6/probe-permissions.ts` phase `auto-classifier-unavailable`, recorded as `perm-auto-classifier-deny`) — so the count is **25 of 33** and eight events stay OPEN. **W7.5/C10.5 closed one more with a run (2026-09-02): `CwdChanged`** had been OPEN since W5 because no phase moved the tracked working directory — the mechanism was never in doubt (the Bash tool appends a `pwd` write to every command and reads it back, so a persisting `cd` moves the tracked cwd and calls the cwd-changed notifier), only the condition was uncreated. The `cwd-change` phase runs one `cd` into a subdirectory of an armed matcher and the event **FIRED on both hook paths**, with the record carrying `old_cwd` and `new_cwd` after the common prefix (`reforge/w5/probe-hook-events.ts` phase `cwd-change`, recorded as `hooks-cwd-change`). **One caller-facing fact came with it:** the notifier that reaches this dispatcher reads the SETTINGS layer and plugin hooks only — never the global store `Options.hooks` callbacks land in — so a callback alone arms nothing and a matcher must be registered through `Options.settings` |
| `permissionMode` values | 6 | **6 characterized** | default/plan/auto/bypass(gated) + `acceptEdits`/`dontAsk` (closeout). **Wave T / probe 99:** a runtime `setPermissionMode` is not guaranteed to take — `auto` off its supported model set and `bypassPermissions` at runtime are both **refused**, and the session stays in its previous mode (no silent fallback to `default`, correcting the earlier reading). Callers must treat the setter as fallible: swap the model before granting `auto`, and report a refusal rather than assuming it applied | **Re-measured 2026-09-01 (W6/C9, pin 2.1.251, `reforge/w6/probe-permissions.ts` + `reforge/research/fixtures/permission-surface-2.1.251.json`):** the enumeration is now derived from the bundle rather than counted — FOUR independent enumerations agree on the six, each member confirmed in comparison position, and upstream's own one-line semantics ship with them. **`bypassPermissions` does NOT short-circuit the rule engine** (the campaign spec and the W5–W7 scout both said it did): the bypass arm is rung 11 of 13 in upstream's pre-check, below the deny rules, the allow rules, the tool's own `checkPermissions` and the ask rules, so a deny rule still bites under bypass and only the ASK is short-circuited — now confirmed by a recording (`perm-bypass-deny-rule`) as well as by the bytes. **And `auto` is NOT gate-dead**, correcting the campaign spec's second delegated unknown: it was ACCEPTED through both paths, at spawn and over the control channel, because upstream's auto gate is three LOCAL conditions (`!circuitBreaker && !settingsDisabled && modelSupportsAuto`) rather than a remote feature flag. **And the classifier under `auto` IS consulted** — the wave's own first reading said otherwise and was corrected on the wire (2026-09-02). A `chmod 600 /etc/hosts` under `auto` was reported as "allowed with no consult", but "no consult" only ever meant no `canUseTool` consult, and the classifier is invisible from the host's seat: watched on the wire, the same call issues a second, toolless, non-streaming `/v1/messages` request that stops at `</severity>` and came back `<severity>25` — below the block threshold, so the tool ran and no host was ever asked. The `classifier` decisionReason is no longer OPEN either: making that request unavailable reaches upstream's fail-closed arm, which denies with `{type:"classifier", classifier:"auto-mode"}` and dispatches the `PermissionDenied` hook on both hook paths (recorded as `perm-auto-classifier-deny`). **What a caller can rely on under `auto`:** every call the acceptEdits fast path does not cover goes to the classifier, an allow verdict reaches the host as silence rather than as a consult, and a classifier the session cannot reach denies rather than allows. This supersedes the Wave T reading above, which measured `auto` on a model that does not support it: the refusal was the model condition, not a gate
| Control-request subtypes (the `control_request` wire) | **54 dispatched by the pinned engine**, over 52 `else if` arms; **37 of them sendable by the installed SDK** (`reforge/research/fixtures/control-protocol-2.1.251.json`, derived from the engine's ladder and from `sdk.mjs` — two artifacts that share no machinery, re-derived on every gate run) | 5 handlers owned in TypeScript (`initialize` and its response payload, `set_permission_mode`, `set_model`, `set_max_thinking_tokens`), plus the two response envelopes W6 took | **Measured 2026-09-02 (W7/C10, `reforge/w7/probe-control-subtypes.ts`): FIRED 38, DEAD 0, OPEN 16 of 54**, one session per subtype on the no-wrapper wire, opened by two controls (a subtype that must answer and a fabricated one that must reach the ladder's terminal `else`) so a broken instrument cannot report a finding. **The wrapper is the reason this was never measured before**: `sdk.mjs` CONSUMES control responses, so an initialize answer, a validation refusal and an unsupported-subtype error reach no surface an SDK-driven session can see — the ~1 KB initialize payload (commands, agents, models, output styles, account shape, permission mode) had never been observed by anything. `reforge/m2/raw-protocol.ts` now sends ten control requests ahead of its prompt and grades each answer on both engines. The 16 OPEN rows each name what creating their condition would cost: an OAuth browser flow, a relay socket, a feedback endpoint, a second model call, or — for `apply_flag_settings` — changing the feature-gate state the whole corpus is graded under. **Two rows were briefly reported DEAD and were not**: `get_workspace_diff` and `register_repo_root` defer through the command-lifecycle wrapper, so an instrument that closed stdin on the same tick ended the session before the answer arrived. **And `get_context_usage` is not free** — its handler counts tokens through 21 further model-side calls of its own |
| System-prompt sections (the preset's `OS()` section list) | **27 dynamic section records + a six-element static head** at pin 2.1.251 (a five-element return array in which exactly ONE element follows the dynamic set; "a two-element tail", quoted on 2026-09-02, miscounted it) (`reforge/research/fixtures/prompt-sections-2.1.251.json`, derived by shape from the section-record constructor and confirmed twice — by the boundary sentinel the block partition looks for, and by the `defaultSystemPrompt` property every caller binds the result to; re-derived on every gate run) | 6 section builders owned in TypeScript (the whole static head: the identity/security opener, `# System`, `# Doing tasks`, `# Executing actions with care`, `# Using your tools`, `# Tone and style`) — about **11.2 KB of the preset's prose** — on top of W3's six assembly functions | **The count was wrong until it was extracted.** "`OS()`'s ~20 prose sections" had been quoted since 2026-09-01 in three places; the pin says 27 + 6 + 2. This is the third population in the reforge campaign counted by hand and found wrong (after the hook events, twice, and the control-protocol arms), and like those it is now a pin-keyed fixture with a gate phase. The 27 dynamic records are NOT all takeable. The fixture carries their SHAPE, not their reasons — the per-record takeability table lives in the reforge README's W7.5 record, because rendering is measured from the cassette while the fixture is pin-keyed. THREE (not four) have nothing to excise: one inline thunk and two cross-chunk producers. Nine of the 27 RENDER on `m1-sysprompt-preset`, so "gated behind experiments, background jobs or remote surfaces a headless corpus never enters" describes only the eighteen that do not; the memory section is a port-heavy S-module and several others are single string constants (a `variable-declarator` or `primitive` shape, not a function). **Three are takeable now** — `session_guidance` (`O8t`, 1,447 B), `context_management` (`G8t`, 291 B) and the `env_info` family — each already rendered by an existing scenario. Every anchor taken is prose occurring ONCE across 1,802 files |
...
>
> **24 of the 33 fire headlessly**, each with a named firing condition the probe creates: the six on an
...
> InstructionsLoaded, UserPromptExpansion, FileChanged, PreModelSwitch and PostModelSwitch — 23 as of
> 2026-09-01, plus **PermissionDenied**, which W6/C9 closed on 2026-09-02 by CREATING its condition
> rather than waiting for it: under `auto`, with the classifier's own request made unavailable,
> upstream's fail-closed arm denies with the exact `decisionReason` the dispatcher is guarded on, and
> the event fired on both hook paths. **No created condition came back dead.** The other 9 stay
> **OPEN**: their conditions are named (an eliciting MCP server, a teammate session, a `--worktree`
> launch, a setup trigger, an interactive config change, a cwd move, `/add-dir` — which refuses
...
> `bypassPermissions` short-circuits the ASK, so no ask-scoped event can fire under it (it does **not**
> skip the rule engine — W6 later measured the bypass arm at rung 11 of the pre-check's thirteen, with
> a deny rule still biting above it, so a probe that wants a rule decision can keep bypass and a probe
> that wants a prompt cannot); a bare `allowedTools` entry **shadows** `canUseTool`, so the callback is never consulted;
