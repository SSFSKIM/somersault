...
| System-prompt sections (the preset's `OS()` section list) | **27 dynamic section records + a six-element static head + a two-element tail** at pin 2.1.251 (`reforge/research/fixtures/prompt-sections-2.1.251.json`, derived by shape from the section-record constructor and confirmed twice — by the boundary sentinel the block partition looks for, and by the `defaultSystemPrompt` property every caller binds the result to; re-derived on every gate run) | 6 section builders owned in TypeScript (the whole static head: the identity/security opener, `# System`, `# Doing tasks`, `# Executing actions with care`, `# Using your tools`, `# Tone and style`) — about **11.2 KB of the preset's prose** — on top of W3's six assembly functions | **The count was wrong until it was extracted.** "`OS()`'s ~20 prose sections" had been quoted since 2026-09-01 in three places; the pin says 27 + 6 + 2. This is the third population in the reforge campaign counted by hand and found wrong (after the hook events, twice, and the control-protocol arms), and like those it is now a pin-keyed fixture with a gate phase. The 27 dynamic records are NOT all takeable and the fixture says why per row: four are inline expressions or cross-chunk dynamic requires with nothing to excise, several are single string constants (a `primitive` capture, not a function), the memory section is a port-heavy S-module, and the rest are gated behind experiments, background jobs or remote surfaces a headless corpus never enters. Every anchor taken is prose occurring ONCE across 1,802 files |
