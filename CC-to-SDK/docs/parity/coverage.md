...
| 2 | **Tool system** — 37 native tools (default-on), `createSdkMcpServer`+`tool()`, allow/deny/`toolAliases`, `toolConfig`, runtime MCP topology | ~75% | ✅ | 3 MCP servers built (tasks/swarm/brief); gating wired; **runtime MCP topology SHIPPED (W3.5)** — `Session.setMcpServers/toggleMcpServer/reconnectMcpServer/mcpServerStatus/setMcpPermissionModeOverride` + daemon `mcp_*` ops + chat-REPL `/mcp` (probes 52/52b: toggle ADVISORY, reconnect stdio-only, add/remove both types); `toolConfig`/`tools` allowlist-shaping partial |
