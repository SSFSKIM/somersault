...
| 10 | **Remote / bridge / voice / UI** — `connectRemoteControl`, remote server, voice, Ink TUI | ~18%¹ | ✅/🚫 | `bridge/` control-protocol shim built; **Phase-3 increment 1 SHIPPED — `cc-harness top`, a read-only terminal daemon-observability dashboard** (lightweight, no-Ink `monitor/`: polls `list`+`context_usage`, renders the live pool / ctx% / token usage / proactive heartbeat with idempotent teardown; CLI-internal, public surface unchanged — spec/plan `2026-06-18-daemon-observability-dashboard`). Interactive control TUI (incr 2) + chat REPL (incr 3) deferred; remote/voice remain 🚫/non-goal **by design** |
