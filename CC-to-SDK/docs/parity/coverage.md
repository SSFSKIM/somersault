...
| 10 | **Remote / bridge / voice / UI** — `connectRemoteControl`, remote server, voice, Ink TUI | ~24%¹ | ✅/🚫 | `bridge/` control-protocol shim built; **Phase-3 increment 1 SHIPPED — `cc-harness top`, a read-only terminal daemon-observability dashboard** (lightweight, no-Ink `monitor/`: polls `list`+`context_usage`, renders the live pool / ctx% / token usage / proactive heartbeat with idempotent teardown; CLI-internal, public surface unchanged — spec/plan `2026-06-18-daemon-observability-dashboard`). **Phase-3 increment 2 SHIPPED — `cc-harness-console`, an interactive Ink daemon console** (new `cc-harness-tui` package over the core's new public `connectDaemon`/`DaemonClient`: master-detail pool/detail, inject prompts via streaming `submit`, drive control ops — interrupt/setModel/setPermissionMode/compact/fork/proactive — with confirm-gated `stop`; spec/plan `2026-06-18-interactive-daemon-console`). Chat REPL (incr 3) deferred; remote/voice remain 🚫/non-goal **by design** |
