...
| External `sessionStore` (S3/Redis/Postgres seam) | ✅ **W3.3**: Redis reference adapter + conformance suite + mirror_error surfacing; cross-host resume live-proven. ✅ **2026-07-30**: `createPostgresSessionStore` (PgLike DI — `pg.Pool`/PGlite as-is; partial-UNIQUE + ON CONFLICT uuid dedup = cross-process retry safety; summary sidecar under seq-CAS; DDL export + idempotent `ensurePostgresSessionStoreSchema`) — full conformance incl. SHOULD-dedup green against real Postgres (PGlite) keylessly; exceeds the official SDK example (no dedup, no summaries there) |
