...
>   hook (probe `104-userpromptsubmit-transcript-path` — domain 8), and a pre-turn `getContextUsage()`
...
>   **Probe-number collision, noted so a later reader is not misled:** `probes/probes/` now holds two
>   different probe 104s — `104-readfile.ts` (the app-server M2b probe that retired `readFile` as dead) and
>   `104-userpromptsubmit-transcript-path.ts` (this wave's). Cite them by filename, not by number.
