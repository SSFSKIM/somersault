...
describe("TaskList", () => {
  ...
  it("ingests activeForm from TaskCreate and owner/activeForm/addBlockedBy from TaskUpdate", () => {
    ...
    for (const m of create("1", "build the parser")) tl.ingest(m);
    for (const m of create("2", "write tests")) tl.ingest(m);
    tl.ingest({ type: "assistant", message: { content: [{ type: "tool_use", id: "tuA", name: "TaskUpdate", input: { taskId: "1", status: "in_progress", activeForm: "Building the parser", owner: "alice" } }] } });
    tl.ingest({ type: "assistant", message: { content: [{ type: "tool_use", id: "tuB", name: "TaskUpdate", input: { taskId: "2", addBlockedBy: ["1"] } }] } });
    expect(tl.snapshot()).toEqual([
  ...
  it("keeps blockers additive and de-duplicated, and carries a TaskCreate activeForm through the result", () => {
    const tl = new TaskList();
    tl.ingest({ type: "assistant", message: { content: [{ type: "tool_use", id: "tc7", name: "TaskCreate", input: { subject: "ship", description: "d", activeForm: "Shipping" } }] } });
    tl.ingest({ type: "user", message: { content: [{ type: "tool_result", tool_use_id: "tc7", content: "Task #7 created successfully: ship" }] } });
    const add = (ids: unknown[]) => tl.ingest({ type: "assistant", message: { content: [{ type: "tool_use", id: `tu${ids.join()}`, name: "TaskUpdate", input: { taskId: "7", addBlockedBy: ids } }] } });
