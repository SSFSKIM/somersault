...
describe("theme.ts", () => {
  ...
  it("highlight.ts names EXACTLY the six jsw scope colors, and no other bare color", () => {
    // F4 Task 3 made highlight.ts a deliberate, single exception to the rule above; F9 T2 widened it from
    // the four-colour lexer's map to canon's FULL fenced-code palette — `jsw` (bundle L523111), also built
    // from CHALK CONSTANTS — so fenced-code colours stay theme-INDEPENDENT upstream and we adopt them
    // verbatim (recorded divergence: no /theme repaint for fenced code, and these six survive under the
    // daltonized themes). The guard is NARROWED, not dropped: any bare colour word beyond these six literals
    // still fails here.
    return readFile(new URL("../../src/tui/highlight.ts", import.meta.url), "utf8").then((source) => {
      ...
      expect(new Set(bare)).toEqual(new Set(['"blue"', '"cyan"', '"green"', '"red"', '"yellow"', '"grey"']));
    });
  });
});
