...
import { createAltScreenGuard, ENTER_ALT, EXIT_ALT, FOCUS_ON, MOUSE_ON_FULL } from "../../src/tui/altScreen.js";
...
describe("ChatApp routes Ctrl-O to the region in fullscreen", () => {
  ...
  it("hands the main screen back around the COMPOSER's editor, and takes the alt screen again after it", async () => {
    ...
    try {
      ...
      expect(writes.slice(child + 1)).toEqual([ENTER_ALT + MOUSE_ON_FULL + FOCUS_ON]);   // …and smcup after it, nothing between
  ...
  it("brackets the plan dialog's ctrl+g editor the same way", async () => {
    ...
    try {
      ...
      r.stdin.write("\x07");
      await waitFor(() => writes.includes(ENTER_ALT + MOUSE_ON_FULL + FOCUS_ON));
      ...
      expect(writes.slice(child + 1)).toEqual([ENTER_ALT + MOUSE_ON_FULL + FOCUS_ON]);
      r.unmount();
    } finally { process.env.VISUAL = ""; ed.clean(); }
  });
  ...
  it("forces a repaint after a round trip whose return frame is byte-identical", async () => {
    ...
    try {
      ...
      const child = writes.indexOf(SPAWN);
      expect(writes.slice(child + 1)).toEqual([ENTER_ALT + MOUSE_ON_FULL + FOCUS_ON, REPAINT]);   // …and smcup's blank screen was painted over
      r.unmount();
    } finally { process.env.VISUAL = ""; ed.clean(); }
  });
  ...
  it("repaints even when the handoff throws", async () => {
    ...
    expect(writes.slice(child + 1)).toEqual([ENTER_ALT + MOUSE_ON_FULL + FOCUS_ON, REPAINT]);     // …and the return leg is exactly these two
    r.unmount();
  });
