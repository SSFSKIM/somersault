...
/** How many of the eight Config rows the frame is currently painting. Matched on the row's GUTTER + label, not
...
const ROW_LABELS = ["Theme", "Model", "Output style", "Default permission mode", "Thinking mode", "Show turn duration", "Reduce motion", "Prompt suggestions"];
...
describe("SettingsDialog — the Config list windows from the height it is given (A6)", () => {
  it("windows the Config list and reports what it clipped", async () => {
    ...
    // The 8 Config rows cannot all fit under a frame this short.
    expect(plain(frame(r.lastFrame))).toMatch(/↓ \d+ more below/);
    expect(shownRows(r.lastFrame)).toBeLessThan(8);
    r.unmount();
  });
  ...
  it("shows every row and neither indicator when the pane is tall enough", async () => {
    ...
    await waitFor(() => frame(r.lastFrame).includes("Theme"));
    expect(shownRows(r.lastFrame)).toBe(8);
  ...
  it("grows the window one row per row of pane, and counts the clipped rows", async () => {
    ...
    expect(plain(frame(a.lastFrame))).toContain(`↓ ${8 - settingsVisibleRows(14)} more below`);
    a.unmount();
    ...
    expect(plain(frame(b.lastFrame))).toContain(`↓ ${8 - settingsVisibleRows(15)} more below`);
    b.unmount();
  });
...
describe("SettingsDialog — the Thinking warning is charged to the window's budget once it is touched", () => {
   ...
   *  last row and steps THREE UP onto `Thinking mode` (W-C T7 put `Show turn duration` below it, F8 T6 put
   *  `Reduce motion` below that, and W-C T12 `Prompt suggestions` below that), and Enter toggles it;
   *  `thinkLevel` stays "default" throughout, so what flips is `thinkingTouched` and not the row's value.
  ...
  const toggleThinking = async (r: ReturnType<typeof render>) => {
    ...
    await waitFor(() => focusedRowLabel(r.lastFrame).startsWith("Prompt suggestions"));
    r.stdin.write("\x1b[A"); r.stdin.write("\x1b[A"); r.stdin.write("\x1b[A");   // …up three → Thinking mode, window unmoved
