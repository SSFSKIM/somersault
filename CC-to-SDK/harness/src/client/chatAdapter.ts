...
import { stageBlocks } from "./stagedSubmit.js";

// The notice moved to `stagedSubmit.ts` with the loop that throws it; re-exported here because
// `useChat.ts` (and its tests) match on it through this module's name.
export { IMAGE_VERSION_SKEW_NOTICE } from "./stagedSubmit.js";
...
export function remoteChatSession(socketPath: string, opts: RemoteChatOpts = {}): RemoteChat {
  ...
  return {
    ...
    // in `stagedSubmit.ts`'s shared loop, staging each one (mint → write bytes → claim) and folding
    // every text block into ONE string for the wire (the `prompt` op has one `text` field, not a block
    // array — the host reassembles the real content array itself once it has read the staged bytes
    // back, `host.ts`'s `assembleStagedContent`).
    async submit(prompt: UserTurnInput, onMessage: (m: unknown) => void) {
      ...
      let text: string;
      let images: { stagedId: string; sha256: string }[] = [];
      ...
      // method reaches — a busy refusal, a dead connection (the staging loop cleans its OWN failures
      // before it throws). Dropped uncalled only once `r.prompt(...)` has actually returned `ok:true`
      // below, past which the HOST owns them.
      let cleanupStaged: () => Promise<void> = async () => {};
      if (typeof prompt === "string") { text = prompt; }
      else {
        let staged: { text: string; images: { stagedId: string; sha256: string }[]; cleanup: () => Promise<void> };
        try { staged = await stageBlocks(prompt, { stageImageOp: (d) => r.stageImageOp(d) }); }
        catch (e) { turnSink = undefined; throw e; }
        ({ text, images } = staged);
      ...
      // ACCEPTED: ownership passes to the host from here (spec v3.1) — `cleanupStaged` is never called
      // again, so there is nothing left in this method that could double-delete a file the host now owns.
