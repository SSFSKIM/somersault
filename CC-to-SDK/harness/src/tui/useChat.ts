...
export function useChat(
  ...
  const [mcpDialog, setMcpDialog] = useState<{ open: boolean }>({ open: false });   // T-MENU task 4
  ...
  async function handleCommand(cmd: ParsedCommand) {
    ...
    try {
      switch (name) {
        ...
        // T-MENU task 4 (spec A2/D5-bl10): a BARE `/mcp` now opens the browser dialog instead of printing
        // `formatMcpStatus`'s text dump — the `reconnect <n>` / `toggle <n> on|off` arg forms are untouched
        // (task-4 brief: "KEEP their current text behavior").
        case "mcp": {
          const action = parseMcpArgs(cmd.args);
          if (!action) { append(formatMcpUsage()); break; }
          if (action.kind === "status") openMcpDialog();
  ...
  // T-MENU task 4 (spec D13's own philosophy: a fresh measurement on open, not a cached one) — the dialog
  // itself calls this once per MOUNT, and every `/mcp` open remounts it fresh (the overlayChain arm below is
  // conditional on `mcpDialog.open`), so no `openSeq`-style cache-busting counter is needed the way
  // SettingsDialog's re-entrant tabs need one.
  function openMcpDialog() { if (!disposed.current) setMcpDialog({ open: true }); }
  function closeMcpDialog() { if (!disposed.current) setMcpDialog({ open: false }); }
  async function fetchMcpServers(): Promise<McpServerRow[]> { return normalizeMcpServers(await session.mcpServerStatus()); }
