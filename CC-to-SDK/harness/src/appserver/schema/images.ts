 ...
 *  It is handler-enforced rather than schema-enforced because `seq` is a number, not a discriminant — the
 *  registry (`imageStage.ts`'s `chunk()`) is that enforcement, and this handler passes its parsed params
 *  straight through with no re-mapping. */
