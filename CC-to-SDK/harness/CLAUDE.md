...
- **`session/`** — lib interactive multi-turn `Session` (`openSession`/`resumeSession`, `.sessionId` capture, compact/control/rewind). `session/chatSession.ts` is the REPL-facing contract (`ChatSession` + the optional `PermissionFeed`/`SessionEvents` mixins) that both the lib `Session` and the remote adapter satisfy structurally. `sessions/` — read API (`listSessions`/`getSessionMessages`) + `forkSession`.
...
- **`client/`** — `chatAdapter.ts` (`remoteChatSession`: a lazily-connecting `ChatSession & PermissionFeed & SessionEvents` over `remote.ts`'s `RemoteChatSession`; the REPL's only session type, used by both the in-process loopback host and `ccx attach`) + `remote.ts`.
- **`tui/`** — the Ink chat REPL, dynamic-imported so headless paths (`-p`, `--bg`) never load React: `chatMain.tsx` is the import target (`runChatClient`), `useChat.ts` is event-driven over a `ChatSession & PermissionFeed & SessionEvents` (turn/message/permission all arrive off the host event stream; `submit`/`answerPermission` are command channels only), plus the rendering/editing components (`ChatApp.tsx`, `Transcript.tsx`, `ChatComposer.tsx`, `PermissionDialog.tsx`, `SessionPicker.tsx`, `ModelPicker.tsx`, `TaskPanel.tsx`) and pure helpers (`render.ts`, `replay.ts`, `commands.ts`, `commandComplete.ts`, `editor.ts`, `fileComplete.ts`, `liveTurn.ts`, `markdown.ts`, `thinkLevels.ts`).
