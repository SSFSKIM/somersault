...
- **`CC-to-SDK/`** — this fork's **own** work (independent of the Rust harness): a TypeScript project replicating Claude Code features on the **Claude Agent SDK** — the harness/product `harness/` (ships the `ccx` binary + library), parity scorecard `docs/parity/`, design docs `docs/superpowers/`, live SDK `probes/`. **Has its own `CLAUDE.md`.**
